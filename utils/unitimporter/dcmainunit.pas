...
uses
  ...
  DCHelpUnit, DCProcCheckUnit, DCImportConsts, ImgList, strUtils;
...
type
  TMainForm = class(TForm)
    ...
    PrepareforimportD20061: TMenuItem;
    ImportDreamD20061: TMenuItem;
    ImportVCLD20061: TMenuItem;
    BitBtn1: TBitBtn;
...
begin
  ...
  try
    ...
    try
      ...
      ProcessForm.StatusMemo.Lines.Text:=fErrText;
      ...
      Else
        ...
        fErrText:=ProcessForm.StatusMemo.Lines.Text+#13#10+fErrText;
...
var i:Integer;
  ...
  ProcessForm : TDCProcessForm;
begin
//  FilesList.Items[0].caption
  fErrText:='';
  lst:=TStringList.Create;
  try
    for I := 0 to FilesList.items.Count - 1 do
     lst.Add(TDCImageListItem(FilesList.Items[ I ] ).Caption);
    ...
    begin
      With TDCImageListItem( FilesList.Items.Add ) Do
          Caption := lst.Strings[ I ];
      If FilesList.ItemIndex = -1 Then
        FilesList.ItemIndex := 0;

      FShowFinalDialog:=false;
      ImportBtnClick(sender);

      FilesList.Items.Clear;
    end;

    fErrText:=trim(ReplaceText(fErrText, '1 файлов было обработано',''));
    
    fErrText:=trim(ReplaceText(fErrText,#13#10#13#10,''));
...
const
  ...
  'designide;dclwbm;dcldss;dclado;dclcds'  //dvd2006
  );
...
begin
  CheckDestPath;
   If Sender=ImportVCLD20061 then
    DelphiVer:=dvD2006
  else
...
var
  DelphiVer:TDelphiVer;
begin
  If (Sender=PrepareforImportD20061) or (Sender=ImportDreamD20061) then
    DelphiVer:=dvD2006
  else
