...
begin
  if S <> nil then
    S.Clear;
  parser:=GetSyntaxParser;
  lst:=TStringList.Create;
  try
    if FindUses(pt, InInterface) then
      while not parser.ParserEOF do
        begin
          parser.NextValidToken;
          if parser.IsTokenChar(';') then
            break;
          s1:=parser.TokenString;
          lst.Add(s1);
//          if (S <> nil) and (parser.TokenString <> ',') then
//            S.Add(parser.TokenString);
        end;
       s1:=AnsiReplaceText(lst.Text, #13#10,'');
       if s<>nil then
       begin
         s.text:=(AnsiReplaceText(s1,',',#13#10));
       end;
      //lst.SaveToFile('d:\11.txt');
  finally
    lst.Free;
  end;
end;
