...
const
  ...
  SDelphiResWords: array[0..cMaxDelphiResWords - 1] of Ansistring = (
  ...
  cMaxDFMResWords = 5;
  SDFMResWords: array[0..cMaxDFMResWords - 1] of AnsiString = (
    'end', 'inherited', 'inline', 'item', 'object');

  cMaxVBScrResWords = 56;
  SVBScrResWords: array[0..cMaxVBScrResWords - 1] of Ansistring = (
  ...
  cMaxJScrResWords = 58;
  SJScrResWords: array[0..cMaxJScrResWords - 1] of Ansistring = (
  ...
  SqlWordsNum = 152;
  SqlResWords: array[0..SqlWordsNum - 1] of AnsiString = (
...
// not coincide with real keywords.
  SPerlScrResWords: array[0..cMaxPerlScrResWords - 1] of Ansistring = (
  ...
  cMaxCppResWords = 94;
  SCPPResWords: array[0..cMaxCppResWords - 1] of Ansistring = ('__asm', '_asm',
  ...
  cMaxCResWords = 31;
  SCResWords: array[0..cMaxCResWords - 1] of Ansistring = ('auto', 'break', 'case',
  ...
  sPythonResWords: array[0..cMaxPythonResWords - 1] of Ansistring =
    ('and', 'assert', 'break', 'class', 'continue', 'def', 'del', 'elif', 'else',
    'except', 'exec', 'finally', 'for', 'from', 'global', 'if', 'import', 'in',
    'is', 'lambda', 'not', 'or', 'pass', 'print', 'raise', 'return', 'try', 'while');

  cDirectivesNum = 13;
  cDirectives: array[0..cDirectivesNum - 1] of ansistring = (
    'DEFINE', 'ELSE', 'ENDIF', 'I', 'IF', 'IFDEF',  'IFEND', 'IFNDEF', 'IFOPT',
    'INCLUDE', 'R', 'RESOURCE', 'UNDEF');

  cCppSuffixesSize = 17;
  cCppSuffixes: array[0..cCppSuffixesSize - 1] of ansistring = ('ui64', 'i64', 'i32', 'i16', 'i8',
...
const
  ...
  SDCPlatformDefine:Array[TDCPlatformKind] of ansiString=(SDCMSWINDOWS,SDCLINUX);
  SDCPlatformDefine2:Array[TDCPlatformKind] of ansiString=(SDCWIN32,SDCLINUX);
  ...
  {$IFDEF D4}
  cAnyChar = [#0..#255];
  cFirstIdentChar = ['A'..'Z', 'a'..'z', '_','À'..'ß','à'..'ÿ'];
  ...
  cFirstIdentChar: TCharSet = ['A'..'Z', 'a'..'z', '_','À'..'ß','à'..'ÿ'];
...
type
  ...
  private
    ...
    procedure SetLinePtr(V: PAnsiChar);
    ...
    function _checksensitiveresword(const s: AnsiString): boolean;

    procedure _Fill2(const StrData: AnsiString; AColorData: pAnsiChar);

    Function SinglePop : Boolean;
  protected
    FTokenPtr: PAnsiChar; // pointer to beginning of the token
    FSourcePtr: PAnsiChar; // current position in source
    FToken: Integer; // current token
    FLinePtr: PAnsiChar; // pointer to the beginning of the line
    FResWordID: Integer;
    function GetErrorDesc: ansiString; virtual;
    procedure DoGetColorData(AToken: Integer; const ATokenStr: ansistring; var AColorData: ansistring); virtual;
    ...
    function ParseEol(C: AnsiChar): Integer;
    function SkipToCharOrEol(C: AnsiChar): boolean;
    ...
    function ParseUniComplexCommentEnd(C: AnsiChar): Integer;
    function ParseJscrComment: Integer;
    function ParseJScrCommentEnd: Integer;
    function ParseUniStrConst(C: ansiChar): Integer;
    ...
    function FindResWord(const s: ansistring): boolean;

    Property TokenList: TCollection Read FTokenList;
  public

    procedure ErrorFmt(const ErrorStr: ansistring; const Args: array of const);
    procedure Error(const ErrorStr: ansistring);
    procedure CheckChar(C: AnsiChar);
    procedure CheckNextChar(C: AnsiChar);
    ...
    function SkipToChar(C: AnsiChar): boolean;

    function NextValidTokenStr: ansistring;
    function PeekValidTokenStr: ansistring;
    function ParserEOF: boolean;
    function NextTokenStr: ansistring;
    function PeekTokenStr: ansistring;
    ...
    function IsTokenChar(C: ansiChar): boolean;
    function IsPeekTokenChar(C: AnsiChar): boolean;
    function IsNextTokenChar(C: AnsiChar): boolean;
    function IsNextTokenResWord(ResWord: Integer): boolean;
    function TokenString: Ansistring; virtual;
    ...
    function ColorString(const StrData: ansiString; InitState: Integer; var AColorData: ansistring): Integer; override;
    ...
    property TokenPtr: PAnsiChar read FTokenPtr;
    property SourcePtr: PAnsiChar read FSourcePtr;
    property LinePtr: PansiChar read FLinePtr write SetLinePtr;
  ...
  protected
    function DefineIsDefined(const ADefine:ansiString):boolean;virtual;
    procedure ParseIncludeFile(const AFileName: ansistring; IncludeInText: Boolean); virtual;
    function IgnoreDefinesAsLine:AnsiString;
  public
    function TokenString: Ansistring; override;
  ...
  TDCFileConfirm = procedure(var AFileName: ansistring; var AConsiderEmpty: Boolean) of object;
...
{-------------------------------------------------------------}
  ...
  protected
    ...
    function GetParsedFileName: ansistring; virtual;
    procedure ParseIncludeFile(const AFileName: ansistring; AIncludeInText: Boolean); override;
    procedure DoFileConfirm(var AFileName: ansistring; var AConsiderEmpty: Boolean); virtual;
  ...
  public
    procedure AddIncludeFile(const FileName: ansistring; IncludeInText: boolean);
    procedure AddIncludeStream(S: TStream);
    procedure LoadUnitPaths(const UnitS: ansistring);
    function NextToken: Integer; override;
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    property ParsedFileName: ansistring read FParsedFileName write FParsedFileName;
  ...
  public
    ...
    function ParseUniPythonStrConstEnd(Achar: ansichar): integer;
  ...
  private
    FStringFinalWord  : ansiString;
    FMultiLineCharSet : TCharSet;

    Procedure WaitForToken;
    Procedure ParserToken;
    Procedure AppendCharSet( Var ACharSet : TCharSet; Const AText : ansiString );
    ...
    Function ParsePerlStrConst(const Ch: ansichar): integer;
  ...
  public
    ...
    function IDToString(ID: Integer): ansistring;
    function StringToID(const Str: ansistring): Integer;
  end;

  {-------------------------------------------------------------}

procedure Register;
function ParseCharSetDef(const S: ansistring): TCharSet;
procedure CreateINTsForDir(const APath: ansistring);
procedure CreateIntFile(const FName: ansistring);
function SearchNotResourcedStrings(const Path, ResultFile, ABadFiles: ansistring;
  ASubFolders: boolean): boolean;
function LoadAndParseIfDefs(const FileName, ADefines, AIgnoreDefines, InclFolders: AnsiString): AnsiString;
function GetFloatFromString(const s: AnsiString): extended;

function DCRemoveIfDefs(const InputString, ADefines, AIgnoreDefines, InclFolders: AnsiString;
  InputStrIsFileName: boolean): AnsiString;
function DCRemoveIfDefsEx(const InputString, ADefines, AIgnoreDefines, InclFolders: AnsiString;
  InputStrIsFileName: boolean; AFileConfirm: TDCFileConfirm): AnsiString;
...
Procedure DCIncPtr( Var P : PAnsiChar );
...
function ResWordHash(const s: AnsiString): integer;
var
  p: pansichar;
  c: integer;
  slen: integer;
...
function GetFloatFromString(const s: Ansistring): extended;
var
  SaveSeparator: ansichar;
begin
  SaveSeparator :=  ansiChar(FormatSettings.DecimalSeparator); //DecimalSeparator;
  try
    formatSettings.DecimalSeparator := '.';
    result := StrToFloat(s);
  finally
    FormatSettings.DecimalSeparator:=char(SaveSeparator);
   // DecimalSeparator := char(SaveSeparator);
  end;
end;
...
function IsValidChar(const c: AnsiChar): boolean;
begin
  result := (c >= #32) or (c = #9);
end;
...
procedure TCustomDCParser.DoGetColorData(AToken: Integer; const ATokenStr: AnsiString;
  var AColorData: AnsiString);
var
  Delta: Integer;
  s1:String;
begin
  ...
  begin
    s1:=AColorData;
    FOnGetTextStyle(Self, AToken, ATokenStr, s1);
    AColorData:=s1;
...
function TCustomDCParser.GetErrorDesc: AnsiString;
begin
  Result := '';
end;
...
procedure TCustomDCParser.ErrorFmt(const ErrorStr: AnsiString; const Args: array of const);
begin
...
procedure TCustomDCParser.Error(const ErrorStr: AnsiString);
begin
  ErrorFmt(ErrorStr, ['']);
end;
...
procedure TCustomDCParser.CheckNextChar(C: AnsiChar);
begin
  NextValidToken;
  CheckChar(C);
end;
...
procedure TCustomDCParser.CheckChar(C: AnsiChar);
var v:Variant;
a:Array of TVarRec;
s:AnsiString;
begin
  ...
  begin
    ...
    setLength(a,1);
    a[0].VInteger:=0;
    a[0].VUnicodeString:=nil;
    a[0].VWideChar:=char(c);
    a[0].VString:=pShortString(s);
    a[0].VChar:=c;
//    ErrorFmt(ResStr(SCharExpected), a);
    Error(ResStr(SCharExpected));
  end;
end;
...
function TCustomDCParser.SkipToChar(C: AnsiChar): boolean;
begin
...
function TCustomDCParser.IsNextTokenChar(C: AnsiChar): boolean;
begin
  NextToken;
...
Function TCustomDCParser.IsTokenChar( C : AnsiChar ) : Boolean;
begin
...
Function TCustomDCParser.IsPeekTokenChar( C : AnsiChar ) : Boolean;
begin
  PushState;
  NextToken;
  Result := IsTokenChar( C );
...
procedure TCustomDCParser.SetLinePtr(V: PAnsiChar);
begin
...
begin
  ...
  for j := 0 to 255 do
    if ansiChar(j) in ACharSet then
...
function TCustomDCParser.SkipToCharOrEol(c: ansichar): boolean;
var
  sptr: pAnsichar;
  spc: integer;
  _c: integer;
begin
  ...
  while (spc <> _c) and IsValidChar(AnsiChar(spc)) do
  begin
    DCIncPtr(sPtr);
    spc := integer(sptr^);
  end;
...
function SkipDigitsEx(sptr: pAnsiChar): pAnsichar;
begin
...
function ParseScaleFactor(sptr: pAnsichar): pAnsichar;
begin
  if sptr^ in ['E', 'e'] then
  begin
    DCIncPtr(sptr);
...
begin
  ...
  repeat
    DCIncPtr(sptr);
  until not (sptr^ in ['0'..'9', 'A'..'Z', '_', 'a'..'z','À'..'ß','à'..'ÿ']);
...
function TCustomDCParser._checksensitiveresword(const s: AnsiString): boolean;
begin
...
function TCustomDCParser.FindResWord(const s: AnsiString): boolean;
var
  i: integer;
begin
  result := false;
...
begin
  ...
  if Result then
  begin
    SetString(StrTmp, pAnsichar(FSourcePtr), 3);
...
function TModuleParser.IgnoreDefinesAsLine:AnsiString;
begin
...
function TModuleParser.TokenString: AnsiString;
var
  P: PAnsiChar;
  i: integer;
  j: integer;
...
function TCustomDCParser.TokenString: AnsiString; // upgrade for the string constant case
begin
...
function TCustomDCParser.ParseEol(C: AnsiChar): Integer;
var
  sptr: pAnsiChar;
begin
...
function TCustomDCParser.ParseUniComplexCommentEnd(C: AnsiChar): Integer;
begin
  while true do
    if SkipToCharOrEol('*') then
    begin
...
function TCustomDCParser.ParseUniStrConst(C: AnsiChar): Integer;
var
  cfound: boolean;
begin
  Result := tokString;
...
function TCustomDCParser.PeekTokenStr: AnsiString;
begin
  PushState;
  Result := NextTokenStr;
  PopState;
...
function TCustomDCParser.NextTokenStr: AnsiString;
begin
  NextToken;
  Result := TokenString;
end;
...
function TCustomDCParser.NextValidTokenStr: AnsiString;
begin
  NextValidToken;
  Result := TokenString;
end;
...
function TCustomDCParser.PeekValidTokenStr: AnsiString;
begin
  PushState;
...
procedure TCustomDCParser._Fill2(const StrData: AnsiString; AColorData: pansiChar);
var
  TokenStr: AnsiString;
  ColorDataStr: AnsiString;
  tstart: integer;
  tslen: integer;
begin
  while SimpleNextToken <> tokEOF do
  begin
    tstart := TokenPtr - pAnsiChar(pointer(StrData)) + 1;
...
function TCustomDCParser.ColorString(const StrData: AnsiString;
  InitState: Integer; var AColorData: Ansistring): Integer;
var
  slen: integer;
  pColorData: integer;
begin
  ...
  begin
    ...
    LinePtr := pansiChar(pointer(StrData));
    slen := length(StrData);
    if length(AColorData) <> slen then
      SetLength(AColorData, slen);

    if Assigned(fOnGetTextStyle) then
      _fill2(StrData, pAnsichar(pointer(AColorData)))
    else
    begin
...
function TPythonParser.ParseUniPythonStrConstEnd(Achar: ansichar): integer;
begin
  while true do
...
begin
  FSourcePtr := PAnsiChar(FLinePtr) + Value;
end;
...
begin
  FTokenPtr := PansiChar(FLinePtr) + Value;
end;
...
procedure TModuleParser.ParseIncludeFile(const AFileName: AnsiString;
...
function TModuleParser.DefineIsDefined(const ADefine:AnsiString):boolean;
var
  Index:Integer;
...
var
  ...
  procedure CheckValid(const ErrorStr: AnsiString);
  begin
...
//    Include( IdentCharSet, '''' );
  Repeat
    DCIncPtr( FSourcePtr );
  Until Not ( FSourcePtr^ In [ '0'..'9', 'A'..'Z', '_', 'a'..'z','À'..'ß','à'..'ÿ', '''' ] );
...
Function TPerlParser.ParsePerlStrConst( Const Ch : ansiChar ) : Integer;
begin
  If ( FLinePtr < FSourcePtr ) And ( ( FSourcePtr - 1 )^ In [ '@', '$', '%', '&' ] ) Then
  begin
    Result := tokWhiteChar;
...
Procedure TPerlParser.AppendCharSet( Var ACharSet : TCharSet; Const AText : AnsiString );
Var
  I : Integer;
begin
  For I := Length( AText ) DownTo 1 Do
    Include( ACharSet, AnsiChar(AText[I]) );
end;
...
Var
  WrapChar                : AnsiChar;
  FinalSymbolStart, Start : PAnsiChar;
begin
  DCIncPtr( FSourcePtr );
  If FSourcePtr^ <> '<' Then
  begin
  ...
  FMultiLineCharSet := [ '0'..'9', 'A'..'Z', '_', 'a'..'z','À'..'ß','à'..'ÿ' ];
  If WrapChar In [ '"', '''' ] Then
    DCIncPtr( FSourcePtr );
...
function TDCIDStringConvert.IDToString(ID: Integer): AnsiString;
begin
  Result := FStrings[ID];
end;
...
function TDCIDStringConvert.StringToID(const Str: AnsiString): Integer;
var
  SaveCount: Integer;
begin
...
var
  ...
  i, FArg, SArg: AnsiChar;
begin
  Result := [];
  Parser := TModuleParser.Create(nil);
  Parser.LinePtr := PansiChar(S);
  with Parser do
  try
            ...
            '.':
              begin
                ...
                if NextToken <> tokString then
                  Error(SErrCharSetDef);
                SArg := TokenString[1];
                for i := ansichar(integer(FArg) + 1) to SArg do
...
function TDCIncludeParser.GetParsedFileName: AnsiString;
begin
...
procedure TDCIncludeParser.AddIncludeFile(const FileName: AnsiString;
  IncludeInText: boolean);
var
  S: TFileStream;
begin
...
procedure TDCIncludeParser.DoFileConfirm(var AFileName: AnsiString; var AConsiderEmpty: Boolean);
begin
...
procedure TDCIncludeParser.ParseIncludeFile(const AFileName: AnsiString; AIncludeInText: Boolean);
var
  FName, _FName: AnsiString;
  I: Integer;
  ConsiderEmpty: Boolean;

  //------------

  function _AddIncludeFile(const FileName: AnsiString): boolean;
  var
    fname: AnsiString;
    SmartExt: AnsiString;
  begin
...
procedure TDCIncludeParser.LoadUnitPaths(const UnitS: AnsiString);
var
  i: Integer;
begin
...
begin
  ...
  ForEachFile(String(Directory) + sPasExt, faAnyFile, SubFolders, CallBack);
...
function SymbolPresent(const s: AnsiString): boolean;
var
  i: integer;
  len: integer;
begin
  ...
  len := length(s);
  for i := 1 to len do
    if s[i] in ['A'..'Z', 'a'..'z','À'..'ß','à'..'ÿ'] then
    begin
      result := true;
      break;
    end;
...
begin
  ...
  begin
    ...
    if InputStrIsFileName then // Dm.
    begin
      LinePtr := PansiChar(DataStr);
      AddIncludeFile(InputString, True);
    end
    else // Dm.
      LinePtr := PansiChar(InputString); // Dm.
