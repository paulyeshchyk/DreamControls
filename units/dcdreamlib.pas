...
{$ENDIF}
  ...
  TSortedList = class(dcClasses.TCustomSortedList);
...
begin
  ...
  while delta > 0 do
    begin
      ...
      //green переменные заполнялись тройками 23,03,2019
      // убрал инициализацию переменных  25,03,2019
      //FillDWORD(0, pv^, sizeof(Variant) * StackDelta div 4);
...
Begin
  Result:=ObjClass;
  Result:=Pointer(PAnsiChar(Result)-4);
  Offset:=-4;
...
begin
  if Assigned(sourceinfo) and Assigned(destinfo) then
    case GetPTypeInfo(sourceinfo).kind of
      ...
      tkString,tkWChar, tkLString, tkUString {$IFDEF D3} ,tkWString {$ENDIF}:
