...
begin
  ...
  begin
    ...
    pc:=TcxPageControl(Component);
    if index=0 then
    begin
      pcPage:=TcxTabSheet.Create({$IFDEF D6}Designer.Root{$ELSE}Designer.Form{$ENDIF});
      try
        pcPage.Parent:=pc;
        pcPage.Name:=Designer.UniqueName(system.copy(pcPage.ClassName, 2, MaxInt));
        pcPage.PageControl:=pc;
      except
        pcPage.Free;
      end;
      pc.ActivePage:=pcPage;
      Designer.SelectComponent(pcPage);
      designer.Modified;
    end else
    begin
      pcPage:=pc.FindNextPage(pc.ActivePage,Index=1,false);
      if (pcPage<>Nil) and (pcPage<>pc.ActivePage) then
      begin
        pc.ActivePage:=pcPage;
        Designer.SelectComponent(pcPage);
        designer.Modified;
