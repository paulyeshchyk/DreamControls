...
uses
  {$IFNDEF CLX}
  ActiveX, windows, messages, shellapi, commctrl,Vcl.ActnList,system.Actions,
  {$ELSE}
  dcActivex,
  {$ENDIF}
...
type
  ...
  private
    ...
    function DoLoadFromStream(FormStream: TStream; Instance: TComponent; needReadRoot:Boolean ): string;
  protected
  ...
  public
    ...
    constructor Create;
    destructor Destroy; override;
    function LoadFromStream(FormStream: TStream; Instance: TComponent;needReadRoot:Boolean=true): string;
    ...
    function CreateFromStream(S: TStream; ModuleClass: TComponentClass; SetDesigning: boolean;readRoot:Boolean=true): TComponent;
...
const
  {$IFDEF D4}
  cOtherIdentChars = ['A'..'Z', 'a'..'z','À'..'ß','à'..'ÿ', '_', '0'..'9'];
  {$ELSE}
  cOtherIdentChars: TCharSet = ['A'..'Z', 'a'..'z','À'..'ß','à'..'ÿ','_', '0'..'9'];
  {$ENDIF}
...
var
  ...
  function CheckIdent(const Ident: AnsiString): boolean;
  begin
...
begin
  ...
  if not result then
    ...
    //green
      if f.HandleAllocated and
        IsWindowVisible(f.Handle) and
        FormIsOk(f)
         then
      begin
        result := true;
...
{green  for i := 0 to ScreenFormCount - 1 do
...
begin
  ...
  if OldSize > 0 then
  begin
    pdef := PMethodDef(pAnsiChar(OldTable) + 2);

    for i := 0 to OldSize - 1 do
    begin
      S.Write(pdef^, pdef^.size);
      pdef := PMethodDef(pansichar(pdef) + pdef^.size);
    end;
  end;
end;
...
function TCompFiler.CreateFromStream(S: TStream; ModuleClass: TComponentClass; SetDesigning: boolean; readRoot:Boolean=true): TComponent;
var
  Hooked: boolean;
begin
...
function TCompFiler.LoadFromStream(FormStream: TStream; Instance: TComponent;needReadRoot:Boolean=true): string;
...
begin
  ...
  begin
    ...
    try
      ...
      MemStream.Seek(0, 0);
      result := DoLoadFromStream(MemStream, Instance, needReadRoot)
    finally
      MemStream.Free;
    end
  end
  else
    result := DoLoadFromStream(FormStream, Instance, needReadRoot);
end;

{--------------------------------------------}

function TCompFiler.DoLoadFromStream(FormStream: TStream; Instance: TComponent;needReadRoot:Boolean ): string;
var
  FPosition: Integer;
  i: Integer;
  Flags: TFilerFlags;
...
function ModulesToListFunc(HInstance: NativeInt; Data: Pointer): Boolean;
begin
  Result := True;
...
begin
  List.Clear;
  {$IFDEF D3}
  EnumModules(ModulesToListFunc, Pointer(List));
  {$ELSE}
...
procedure SafeRegisterActions(const CategoryName: string; const AClasses: array of TBasicActionClass; Resource: TComponentClass);
begin
  if Assigned(system.Actions.RegisterActionsProc) then
    system.Actions.RegisterActions(CategoryName, AClasses, Resource);
end;
{$ENDIF}
