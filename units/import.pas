...
const
  BadProcCount = 159+13;
  BadProcList : array[0 .. BadProcCount - 1] of Ansistring =
(
...
const
  BadTypeCount = 11+13;
  BadTypeList : array[0 .. BadTypeCount - 1] of ansistring =
(
//  cArrayOfConst,
 'DataSources',
 'DRObject',
...
const
  cSysUnits : array[0..4] of ansistring =
(
...
var
  ...
  function GetShortParamStr(ParamIndex, ParamCount : integer;
    recparams : TStringList; UsepArgs : boolean;useOriginalParamNames:Boolean=true) : string;
  var
    k : integer;
    HasParams : boolean;
    isRecParam : boolean;
      ...
      begin
        with PParamInfo(ModuleInfo.GlobalParams[ParamIndex + k])^ do
          begin
            ...
            begin
              if useOriginalParamNames then
              AddStr(result, ModuleInfo.GetGlobalString(ParamName)) else
  ...
  function GetParamStr(const AddInst : string; ParamIndex, ParamCount : integer;AddFullParamsName:Boolean=true) : string;
  var
    k : integer;
    pinfo : PParamInfo;
    fstr : string;
        ...
        with pinfo^ do
          begin
            ...
            if ptype <> '' then
              ...
              ss := '';
            if AddFullParamsName then
            AddStr(result, fstr + ModuleInfo.GetGlobalString(ParamName) + ss) else
  ...
  function GetProcDesc(pminfo : PMethodInfo; const AddInst : string;AddFullParamsName:Boolean=true) : string;
  begin
    with pminfo^ do
      begin
        result := GetParamStr(AddInst, ParamIndex, ParamCount,addFullParamsName);
    ...
    with RFuncWrappers, pminfo^ do
      begin
        ...
        if defparam > 0 then
          ...
          procdesc := GetProcDesc(pminfo, cname,false);
        ...
        try
            ...
            with ModuleInfo do
              begin
                ...
                if IsRecType(ModuleInfo,ptype) then
                  ...
                  k:=SetParams.AddObject(ptype, pointer(i));
              end;
          ...
          repeat
            ...
            AddStr(s, GetShortParamStr(ParamIndex, pCount, RecParams, defparam > 0,false));
