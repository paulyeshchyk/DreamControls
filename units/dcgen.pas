...
uses
  ...
  Messages, Forms, Classes, StdCtrls,extctrls,dcClasses, system.types,
  {$IFDEF D6}
  RTLConsts,inifiles,
  {$ENDIF}
...
procedure TFDesigner.CanInsertComponent(AComponent: TComponent);
begin
//
end;
...
begin
  if (Operation = opInsert) and
    ...
    BroadCastToNotifiers(self, CM_OBJINSERT, integer({$IFNDEF D4}AComponent{$ELSE}AnObject{$ENDIF}), 0) else
  if (Operation=opRemove) then
  BroadCastToNotifiers(self,CM_OBJREMOVE, integer({$IFNDEF D4}AComponent{$ELSE}AnObject{$ENDIF}), 0);
end;

{------------------------------------------------------------------}

function TFDesigner.GetDesignerExtension:String;
begin
...
{------------------------------------------------------------------}
procedure TFDesigner.PaintMenu;
begin
...
{------------------------------------------------------------------}
 {$IFDEF USEVCLDESIGNER }
function TFDesigner.CreateMethod(const Name: string; const AEventInfo: IEventInfo): TMethod;
var
  ownercomp : TComponent;
  pdata : pointer;
begin
...
{------------------------------------------------------------------}
 ...
 {$IFDEF USEVCLDESIGNER }
procedure TFDesigner.GetMethods(const AEventInfo: IEventInfo; Proc: TGetStrProc);
begin
...
{$ENDIF}
 ...
 {$IFDEF USEVCLDESIGNER }
procedure TFDesigner.SelectComponent(const ADesignObject: IDesignObject);
begin
...
begin
  ...
  {$ENDIF}
  dcStrlEd.RunRegister; //green ïåðåíåñåíî èç dcStrled
  glyphed.RunRegister;//
end;
...
procedure UpdateDisabledControls(AOwner: TComponent);
function HasParentForm(Component:TWinControl):boolean;
var p1:TwinControl;
begin
//
result:=false;
p1:=Component;
if p1.ClassName='TForm' then
begin
  result:=true;
  exit ;
end;
...
begin
{<green>}
{if dcidedes.isClosing then
begin
  if (dcidedes.FormNo=0) then
  begin
    inc(dcidedes.FormNo);
    exit;
  end;
...
procedure _Iterat(const Path: string);
  ...
  begin
    ...
    try
      ...
      begin
        FileName := Strings[I];
       //vadim FileName := ExpandFileName(FileName);
...
function ToolsFormStyle: TFormStyle;
const
  styles: array[boolean] of TFormStyle = (fsNormal{fsMDIChild}, fsStayOnTop);
begin
