...
uses
  ...
  {$ENDIF}
  dcsystem, SysUtils, Classes, typinfo, dccommon, dcdreamlib, dccontrols, dcconsts,cxDropDownEdit,
  cxContainer
  ;
...
//    popupContainer:TcxPopupEditPopupWindow;
  ...
  public
    constructor Create(AOwner: TComponent); override;
    constructor CreateSizeable(AOwner: TComponent);
    procedure CreatePanels;
    procedure InitColumnSizes;
...
function PopupListboxEx2(AParent: TWinControl; Sl: TStrings;Values:Tstrings; X, Y: integer;
  AlignArray: array of TLeftRightAlign;
  AFont: TFont;
...
const cxEditMouseSizingCursorMap: array[-1 .. 1, -1 .. 1] of TCursor = (
...
begin
  ...
  with Params do
  begin
    Style := WS_CHILD or WS_BORDER or WS_VSCROLL or WS_HSCROLL or LBS_NOTIFY or LBS_HASSTRINGS or
...
begin
  Perform(LB_SETCURSEL, Perform(LB_FINDSTRING, 0, Msg.LParam), 0);
  AdjustHeight;
end;
...
begin
  ...
  begin
    ...
    begin
     ...
     {$IFNDEF CLX}
      Windows.DrawText(Canvas.Handle, Text, StrLen(Text), Rect, DT_SINGLELINE or
...
begin
  ...
  begin
    ...
    begin
      Rect.Left := Rect.Left + ColorWidth + 6;
      {$IFNDEF CLX}
      Windows.DrawText(Canvas.Handle, PChar(BrushNames[index + 1]), Length(BrushNames[index + 1]), Rect, DT_SINGLELINE or
        DT_VCENTER or DT_NOPREFIX);
      {$ELSE}
...
begin
  ...
  begin
    ...
    begin
      ...
      {$IFNDEF CLX}
      Windows.DrawText(Canvas.Handle, PChar(s), Length(s), Rect, DT_SINGLELINE or
        DT_VCENTER or DT_NOPREFIX);
      {$ELSE}
  ...
  begin
    ...
    begin
      ...
      if FAlign = lrRightJustify then
        dec(ARect.Right);
      dcControls.DrawWithSelection(Canvas, Canvas.Font, FBoldFont, ARect, s, BPos, BLen, FAlign);
...
{------------------------------------------------------------------}
 procedure TdcPopupListBox.InitColumnSizes;
 var
  i: integer;
  Pos: integer;
  Cnt: integer;
 ...
 begin
  FInternalList.Clear;
  aCanvas:=Canvas;
  s:='function ~TCustomOPPForm.SomeCustomCreateAction(p1,p2,p10:String)~:String;';
  size := CalcSize(ACanvas,s);
  FInternalList.Add('');
  FInternalList.Objects[0] := pointer(size);
  FInternalList.Add('');
  FInternalList.Objects[1] := pointer(size);
...
begin
  ...
  ACanvas := Canvas;
  {$ENDIF}
  if fCustomDraw and (FUpdateCount <= 0)and (not(csUpdating in self.ComponentState)) then
    for i := 0 to Items.Count - 1 do
    begin
      Pos := 1;
      Cnt := 0;
...
{******************************************************************}
constructor TDCPopupListBox.CreateSizeable(AOwner: TComponent);
begin
  fCustomDraw := true;
  {$IFNDEF CLX}
...
procedure TdcPopupListBox.CreatePanels;
begin
  rPanel:=TPanel.Create(self);
  rPanel.Parent:=self;
  rPanel.BevelOuter:=bvNone;
  rPanel.BevelKind:=bkTile;
  rPanel.BevelInner:=bvRaised;
  rPanel.Width:=10;
  rPanel.Cursor:=crSizeWE;
  rPanel.Align:=alRight;

  downPanel:=TPanel.Create(self);
  downPanel.Parent:=self;
  downPanel.Align:=alBottom;
  downPanel.BevelOuter:=bvNone;
  downPanel.BevelKind:=bkTile;
  downPanel.BevelInner:=bvRaised;
  downPanel.Height:=10;
  downPanel.Cursor:=crSizeNS;
end;
...
begin
  ...
  {$ENDIF}
  if Screen<>Nil then
  if sizeX>Screen.Width div 3 then
  SizeX:=Screen.Width div 3;
...
var
  ...
  s:String;
begin
  Result := TDCPopupListBox.CreateSizeable(AParent);
  Result.Updating;
//  result.CustomDraw:=false;
  with Result do
  begin
...
//    CreatePanels;
    result.Updated;
   //result.ResetColumnSize;
    result.InitColumnSizes;
    Popup(p.x, p.y);
  end;
end;
