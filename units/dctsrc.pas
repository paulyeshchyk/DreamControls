...
function getNewCollectionItem:TCollectionItem;
 var Meth : TRttiMethod;
  Para : TRttiParameter;
  Param : TArray<TValue>;
  info:TRttiType;
//  ARec : TRecordType;
...
begin
  ...
  try
    Info := ctx.GetType( fCollection.ClassInfo);
    Meth := Info.GetMethod('Add');
    Setlength(Param, 0);
    o:=Meth.Invoke(fCollection, Param);
    if fCollection.Count>0 then
    result:=fcollection.Items[FCollection.Count-1];
  finally
    ctx.Free;
  end;
end;
