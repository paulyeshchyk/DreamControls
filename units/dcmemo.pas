...
{$IFNDEF CLX}
  Messages, Windows, Forms, Dialogs, CommCtrl, Graphics, controls, StdCtrls,
  clipbrd, RichEdit, menus,
{$ELSE}
  Types, Qt, QForms, Qdialogs, QGraphics, QControls, Qstdctrls, QExtCtrls,
  QClipbrd, QTypes, QMenus,
{$ENDIF}
  dcsystem, consts, Classes, SysUtils, dcdreamlib, dcedit, dccommon, typinfo,
  dccontrols,
...
type
  ...
  TMemoOption = (moDrawMargin, moDrawGutter, moThumbTracking, moDblClickLine,
    moColorSyntax, moLineNumbers,
    moOverwriteCaret, moLeftIndent, moDisableDrag, moSelectOnlyText,
    moLineNumbersOnGutter, moNotePadCursorStyle,
    moBreakWordAtMargin, moDrawSpecialSymbols, moDisableRightClickMove,
    moColoredLineStyle, moDisableInvertedSel,
    moHideInvisibleLines, moDrawLineBookmarks, moShowScrollHint,
    moUseReadOnlyColor, moExtendedSel, moCenterOnBookmark,
  ...
  published
    ...
    property UpdateMemoColors: TUpdateMemoColors read FUpdateMemoColors write
      SetUpdateMemoColors;
    ...
    property UpperCase: boolean read FUpperCase write SetUpperCase default
      false;
  end;
  ...
  public
    ...
    function AddStyle(const StyleName: string; AColor, ABackColor: TColor):
      TTextStyle;
    function AddStyleEx(const StyleName: string; AColor, ABackColor: TColor;
      AStyle: TFontStyles;
      AUpdateType: TUpdateMemoColors; AUseMemoFont, AUseMemoColor: boolean):
      TTextStyle;
  ...
  published
    ...
    property BookmarkIndex: integer read FBookmarkIndex write SetBookmarkIndex
      stored IsBoomarkIndexStored;
  end;
  ...
  public
    procedure AddFromResource(const ResName, GutterImageName: string;
  ...
  public
    function Compare(Item1, Item2: Pointer): integer; override;
    function FindNear(Item: PWBPoint; CheckIndex: boolean; var Exact: boolean):
      integer;
  ...
  public
    ...
    property ResetOnOverFlow: boolean read GetResetOnOverFlow write
      SetResetOnOverFlow;
  end;
  ...
  private
    ...
    procedure SetStrAttribute(LinePos, Left, Right: integer; Attribute:
      TMemoAttributes);
    procedure GetStrAttribute(LinePos, Left, Right: integer; Index: byte;
      var ConsistentAttrs: TConsistentAttrs);
    procedure GetAttributes(ARect: TRect; SelectionType: TSelectionType;
      Attribute: TMemoAttributes;
      var ConsistentAttrs: TConsistentAttrs);
    procedure SetAttribute(ARect: TRect; SelectionType: TSelectionType;
      Attribute: TMemoAttributes);
  ...
  public
    ...
    function GetBackColor(AState: integer; var Color: TColor): boolean;
      override;
    function ColorString(const StrData: ansistring; InitState: integer;
      var AColorData: ansistring): Integer; override;
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure PrepareColorData(LinePos: integer; const s: ansistring; var ColorData:
      ansistring); override;
    function StoreColorData: boolean; override;
    procedure ClearStyles;
  published
    property ResetOnOverFlow: boolean read FResetOnOverFlow write
      FResetOnOverFlow;
  end;
...
{$ENDIF}
  ...
  TDrawState = (dsBeforePaint, dsAfterPaint, dsText, dsAfterText, dsSelection,
    dsMarkedText);
  TDrawStates = set of TDrawState;
  TPrintType = (ptHeader, ptFooter);

  THintPopup = procedure(Sender: TObject; Strings: TStrings; var AllowPopup:
    boolean; var PopupType: TPopupType) of object;
  TTemplateInsertEvent = TStringsEvent;
  TMemoChangeEvent = procedure(Sender: TObject; State: TMemoStates) of object;
  TClickGutterEvent = procedure(Sender: TObject; LinePos, ImageIndex: integer;
    Shift: TShiftState) of object;
  THintInsertEvent = procedure(Sender: TObject; var s: string) of object;
  TMemoScrollEvent = procedure(Sender: TObject; ScrollStyle: TScrollStyle;
    Delta: integer) of object;
  TVertScrollEvent = procedure(Sender: TObject; TopLine: integer; EndScroll:
    boolean; var ShowScrollHint: boolean; var ScrollHint: string) of object;

  TGetColorStyleEvent = procedure(Sender: Tobject; APoint: TPoint; var AStyle:
    integer) of object;
  TJumpToUrlEvent = procedure(Sender: TObject; const s: string; var Handled:
    boolean) of object;

  TCollapseStateChanging = procedure(Sender: TObject; LinePos: integer;
    ACollapsed: boolean; var Handled: boolean) of object;
  TCollapseStateChanged = procedure(Sender: TObject; LinePos: integer;
    ACollapsed: boolean) of object;

  TCustomStringEvent = procedure(Sender: TObject; var s, ColorS: string) of
    object;
  TCustomDrawEvent = procedure(Sender: TObject; Canvas: TCanvas; const Rect:
    TRect; LinePos, CharPos: integer; const S: string;
    DrawState: TDrawStates; var Handled: boolean) of object;

  TDrawGutterEvent = procedure(Sender: TObject; Canvas: TCanvas; const Rect:
    TRect; DrawState: TDrawState; var OwnerDraw: boolean) of object;
  TExportStringEvent = procedure(Sender: TObject; var s: string) of object;
  TFindStringInPopupEvent = procedure(Sender: TObject; Strings: TStrings; const
    s: string; var Index: integer; var Handled: boolean) of object;
  TGetLineNumberEvent = procedure(Sender: TObject; var s: string; var MaxLen:
    integer) of object;

  TGetFontFromStyleEvent = procedure(Sender: TObject; Style: integer; Font:
    TFont; var Color: TColor) of object;
  TBeforePrintEvent = procedure(Sender: TObject; var s: string; AType:
    TPrintType) of object;
  ...
  private
    ...
    fBeginEndColors:Array of TColor;
...
{$ENDIF}
    ...
    procedure CalcGutterImgSize(Index: integer; var NewWidth, NewHeight:
      integer);
    ...
    function SetColorFromStyle(AColorStyle, AOldStyle, AOldColor, AOldBkColor:
      integer;
      InSelection, ASkipColor, ACalcRect: boolean): TFont;
    procedure GetSelColorsFromStyle(Style: integer; var AColor, ABkColor:
      integer);
    procedure _TextOut(var R: TRect; BaseLine, CurLine, CurWrapLine, CurChar, X,
      Y: integer;
      PStr: PChar; Len: integer; CalcRect, ASelection, AMarkedText: boolean);
    procedure _FillRect(ARect: TRect; CurLine, CurWrapLine, CurChar, ABeginSel,
      AEndSel, AOldStyle: integer);

    function PaintString(var R: TRect; var BaseLine: integer; const S, ColorS:
      string;
      begChar, endChar, CurLine, CurWrapLine, CurPos, CurStyle: integer;
      SkipColors, CalcRect, CalcSymbols: boolean): integer;

    function CalcStringSize(S, ColorS: string; Pos, Len: integer; var BaseLine:
      integer): TPoint;
    function SymbolsDrawn(LinePos, CharPos: integer; const S, ColorS: string; var
      BaseLine: integer;
    ...
    function IsUrlUnderPoint(Pt: TPoint; var s: string; TestOnly: boolean):
      boolean;
    ...
    procedure FontAndColorsFromStyle(Style: integer; var Font: TFont; var Color,
      BkColor: integer);
    ...
    function PixelToTextLine(var LinePos, CharPos: integer; CheckEnd: boolean):
      integer;
    function PixelToTextChar(LinePos, CharPos, X: integer; CheckEnd: boolean):
      integer;
    ...
    // rtf support
    procedure SetClipboardBuffer(Format: {$IFNDEF CLX}Word{$ELSE}string{$ENDIF};
      var Buffer; Size: Integer);
    procedure GetClipboardBuffer(Format: {$IFNDEF CLX}Word{$ELSE}string{$ENDIF};
      Stream: TStream);
{$IFDEF CLX}
    procedure OnHintTimer(Sender: TObject);
    procedure OnDragTimer(Sender: TObject);
    procedure PaintScrollRect;
    procedure QTextOut(ACanvas: TCanvas; const R: TRect; Ptr: PChar; Len:
      integer; AFontInfo: PFontInfo; AFill: boolean; Buffer: Pointer);
    procedure QBitBlt(dst: TCanvas; dx, dy: integer; src: TCanvas; sx, sy, sw,
      sh: integer);
    procedure QFillRect(ACanvas: TCanvas; const R: TRect; ABrush: TBrush);
{$ELSE}
    procedure DoTextOut(ACanvas: TCanvas; R: TRect; Style: integer; Ptr: PChar;
      Len: integer; Buffer: Pointer; OverHang: integer);
{$ENDIF}
    procedure ImportText(Sender: TObject; const s: string; AFont: TFont; AColor:
      TColor);
    ...
    function FindWordBreak(LinePos: integer; var Pos, BaseLine: integer; var R:
      TRect): string;
    ...
    function GetWBPositionEx(Position: TPoint; CheckMBCS, ConvertResult:
      boolean; var Pos: integer): TPoint;
    //    procedure CheckLastChar(var Index: integer ; AX : integer);

    function GetRealPoint(Pt: TPoint): TPoint;

    function CalcWrapPos(LinePos, CharPos: integer; var APoint: TPoint):
      boolean;
    function GetRealStrCount: integer;
    function GetRealStrCountEx: integer;
    function GetWBStringLength(RealIndex, Index: integer; CheckLineEnd:
      boolean): integer;
    ...
    function GetWrappedLineHeightEx(Index: integer; var Pos, BaseLine: integer;
      SingleLine, Direction: boolean): integer;
    function GetWrappedLineWidth(Index: integer; var Pos: integer; SingleLine:
      boolean): integer;
    ...
    procedure PopupBoxKeyDown(Sender: TObject; var Key: Word; Shift:
      TShiftState);
...
{$ENDIF}
    ...
    function DoPaintGutter(const ARect: TRect; DrawState: TDrawState): boolean;
      virtual;
...
{$ENDIF}
    ...
    procedure DoGetFontFromStyle(Style: integer; Font: TFont; var Color:
      TColor);
      virtual;
    ...
    function CollapseStateChanging(LinePos: integer; State: TCollapseState):
      boolean; virtual;
    procedure CollapseStateChanged(LinePos: integer; State: TCollapseState);
      virtual;
    function DoFindInPopupBox(AStrings: TStrings; const s: string; var Index:
    ...
    function ClosePopupListBox(var Key: Word; Shift: TShiftState): boolean;
      virtual;
...
{$ENDIF}
  ...
  public
    ...
    procedure FindInPopupBox(Sender: TObject;BeginStr:String);

    property  _PopupListBox: TDCPopupListBox read FPopupListBox;
    property _HintPos:TPoint read FHintPos write fHintPos;
    property HintPopup: TStrings read FHintPopup;
   // property _PopupListBox: TDCPopupListBox read FPopupListBox;
...
{$ENDIF}
    procedure TempHighlightLineWithStyle(Index: integer; const StyleName:
      string);
...
{$ENDIF}
    ...
    procedure ExportToStreamEx(Stream: TStream; Exporter: TDCExporterClass;
      Selection: boolean);
    procedure ExportToFileEx(const FileName: string; Exporter: TDCExporterClass;
      Selection: boolean);
    ...
    procedure PasteFromImportStream(Stream: TStream; ImporterClass:
      TDCImporterClass);
    ...
    procedure ShowPopupListBoxEx(Strings: TStrings; AlignArray: array of
      TLeftRightAlign; AMaxSize: integer);
    procedure ShowPopupListBoxEx2(Strings: Tstrings; Values: Tstrings);
    ...
    property SelColor: TColor read FSelColor write SetSelColor default
      clHighlightText;
    property SelBackColor: TColor read FSelBackColor write SetSelBackColor
      default clHighlight;
    property MatchColor: TColor read FMatchColor write SetMatchColor default
      clLime;
    property MatchBackColor: TColor read FMatchBackColor write SetMatchBackColor
      default clWindowText;

    property HideSelection: boolean read FHideSelection write SetHideSelection
      default false;
    property SpacesInTab: integer read GetSpacesInTab write SetSpacesInTab;
    property HorzMaxPos: THorzMaxPos read FHorzMaxPos write SetHorzMaxPos default
      512;
    property TextStyles: TTextStyles read FTextStyles write SetTextStyles stored
      FTextStylesChanged;
    property MemoSource: TCustomMemoSource read FMemoSource write SetSource;
    property BorderStyle: TBorderStyle read FBorderStyle write SetBorderStyle
      default bsSingle;
    property UseGlobalOptions: boolean read GetUseGlobalOptions write
      SetUseGlobalOptions;
    property ScrollBars: TScrollStyle read FScrollBars write SetScrollBars
      default ssNone;
    property Options: TMemoOptions read FOptions write SetOptions;
    property MarginPos: TMarginPos read FMarginPos write SetMarginPos default
      80;
    ...
    property GutterLineColor: TColor read FGutterLineColor write
      SetGutterLineColor default clGray;
    property GutterWidth: TGutterWidth read FGutterWidth write SetGutterWidth
      default 32;
    property GutterImgs: TDCGutterImages read FGutterImgs write SetGutterImgs
      stored FGutterImagesChanged;
    property GutterImgsAlign: TGutterImgsAlign read FGutterImgsAlign write
      SetGutterImgsAlign default iaLeft;
    property LineNumbersWidth: integer read FLineNumbersWidth;
    property ClipboardType: TClipboardType read FClipboardType write
      SetClipboardType default cbNormal;
    property PlainText: boolean read GetPlainText write SetPlainText default
      true;
    property OnStateChange: TMemoChangeEvent read FOnStateChange write
      FOnStateChange;
    ...
    property OnClickGutter: TClickGutterEvent read FOnClickGutter write
      FOnClickGutter;
    property OnHintPopup: THintPopup read FOnHintPopup write FOnHintPopup;
    property OnTemplateInsert: TTemplateInsertEvent read FOnTemplateInsert write
      FOnTemplateInsert;
    property OnHintInsert: THintInsertEvent read FOnHintInsert write
      FOnHintInsert;
    property OnCreateDialog: TCreateDialogEvent read FOnCreateDialog write
      FOnCreateDialog;
    property OnMemoScroll: TMemoScrollEvent read FOnMemoScroll write
      FOnMemoScroll;
    property OnVerticalScroll: TVertScrollEvent read FOnVerticalScroll write
      FOnVerticalScroll;
    property OnGetColorStyle: TGetColorStyleEvent read FOnGetColorStyle write
      FOnGetColorStyle;
    property OnMouseLeave: TNotifyEvent read FOnMouseLeave write FOnMouseLeave;
    property OnGetFontFromStyle: TGetFontFromStyleEvent read FOnGetFontFromStyle
      write FOnGetFontFromStyle;
    property OnCustomString: TCustomStringEvent read FOnCustomString write
      FOnCustomString;
    property OnCustomDraw: TCustomDrawEvent read FOnCustomDraw write
      FOnCustomDraw;
    property OnDrawGutter: TDrawGutterEvent read FOnDrawGutter write
      FOnDrawGutter;
    property OnCollapseStateChanging: TCollapseStateChanging read
      FOnCollapseStateChanging write FOnCollapseStateChanging;
    property OnCollapseStateChanged: TCollapseStateChanged read
      FOnCollapseStateChanged write FOnCollapseStateChanged;
    property OnExportString: TExportStringEvent read FOnExportString write
      FOnExportString;
    property OnFindStringInPopup: TFindStringInPopupEvent read
      FOnFindStringInPopup write FOnFindStringInPopup;
    property OnGetLineNumber: TGetLineNumberEvent read FOnGetLineNumber write
      FOnGetLineNumber;
    property OnBeforePrint: TBeforePrintEvent read FOnBeforePrint write
      FOnBeforePrint;

    property ReadOnly: boolean read GetReadOnly write SetReadOnly stored
      GetSourcePropsStored;
    property Lines: TStrings read GetLines write SetLines stored
      GetSourcePropsStored;
    ...
    property StringsOptions: TStringsOptions read GetStringsOptions write
      SetStringsOptions stored GetSourcePropsStored;
    property SyntaxParser: TSimpleParser read GetSyntaxParser write
      SetSyntaxParser stored GetSourcePropsStored;
    property TabStops: string read GetTabStops write SetTabStops stored
      GetSourcePropsStored;
    property Delimeters: string read GetDelimiters write SetDelimiters stored
      GetSourcePropsStored;
    property AllowUndo: boolean read GetAllowUndo write SetAllowUndo default
      true;
    ...
    property GutterBackColor: TColor read FGutterBackColor write
      SetGutterBackColor;
    property LineBookmarkColor: TColor read FLineBookmarkColor write
      SetLineBookmarkColor default clRed;

    property Text: {$IFNDEF CLX}string{$ELSE}TCaption{$ENDIF} read GetText write
    SetText;
    property SelText: string read GetSelText write SetSelText;
    property OnChange: TNotifyEvent read FOnChange write FOnChange;
    property PrintOptions: TDCPrintOptions read FPrintOptions write
      FPrintOptions;
    property CodeTemplates: TMemoCodeTemplates read GetCodeTemplates write
      SetCodeTemplates;
    ...
    property PrintIndent: integer read FPrintIndent write FPrintIndent default
      0;
    property MemoBackground: TControlBackground read FMemoBackground write
      SetMemoBackground;
    ...
    property GutterBackground: TControlBackground read FGutterBackground write
      SetGutterBackground;
    property LineNumBackColor: TColor read FLineNumBackColor write
      SetLineNumBackColor default clWindow;

    property HideScrollBars: boolean read FHideScrollBars write SetHideScrollBars
      default false;
    property HighlightUrls: boolean read GetHighlightUrls write SetHighlightUrls
      default false;
    property WordWrap: boolean read FWordWrap write SetWordWrap default false;
    property BlockOption: TBlockOption read GetBlockOption write SetBlockOption
      default bkBoth;
    property OutFormat: TOutFormatType read GetOutFormat write SetOutFormat
      default ofWindows;
    property LineSeparator: TLineSeparator read FLineSeparator write
      SetLineSeparator;
    property LineHighlight: TLineHighlight read FLineHighlight write
      SetLineHighlight;
    property SpecialSymbols: TSpecialSymbols read FSpecialSymbols write
      SetSpecialSymbols;
    property UseDefaultMenu: boolean read FUseDefaultMenu write FUseDefaultMenu;

    property MaxLineLen: integer read GetMaxLineLen;
    property GutterLineIndent: integer read FGutterLineIndent write
      SetGutterLineIndent default 0;
    property HintPause: integer read FHintPause write FHintPause;
    property WordSelectionType: TWordSelectionType read FWordSelectionType write
      FWordSelectionType default wsWordOnly;
    property UseMonoFont: boolean read FUseMonoFont write SetUseMonoFont default
      true;
    property SelAttributes: TMemoAttributes read GetSelAttributes write
      SetSelAttributes;
  published
...
{$ENDIF}
  ...
  protected
    procedure cmSaveColorStyles(var Message: TMessage); message
      cm_SaveColorStyles;
    procedure cmLoadColorStyles(var Message: TMessage); message
      cm_LoadColorStyles;
  public
  ...
  published
    ...
    property UseMonoFont: Boolean read FUseMonoFont write SetUseMonoFont default
      True;
    property ReadOnlyColor: TColor read FReadOnlyColor write SetReadOnlyColor;
    property SpecialSymbols: TSpecialSymbols read FSpecSymbols write
      SetSpecSymbols;
...
{$IFNDEF CLX}
procedure ImageListToGutterImages(ImageList: TImageList; GutterImages:
  TDCGutterImages);
{$ENDIF}
...
procedure PaintGutterLine(Canvas: TCanvas; R: TRect; X: integer; ABkColor,
  AColor: TColor);
begin
  with Canvas, R do
  begin
...
function TWordWrapList.FindNear(Item: PWBPoint; CheckIndex: boolean; var Exact:
  boolean): integer;
var
  ALine: integer;
begin
      ...
      while (result >= 0) and (result < Count) do
        ...
        begin
          ...
          if ALine < Item^.Y then
            break
          else if CheckIndex then
          begin
            if Exact then
              break;
          end
          else if Exact and (X <= Item^.X) then
            break;
...
begin
  ...
  if NeedPaint then
    ...
    begin
      ...
      PaintGutterLine(Self.TempBackground.Canvas, R, X, FGutterBackColor,
        FGutterLineColor);
    end;
end;
...
begin
  Filer.DefineBinaryProperty('EOLStringBinary', ReadEOLStringData,
    WriteEOLStringData, True);
  Filer.DefineBinaryProperty('EOFStringBinary', ReadEOFStringData,
    WriteEOFStringData, True);
end;
...
begin
  ...
  if ReadIntegerDialog(SCapGotoLineNumber, SCapEnterNewLine, 0, Self,
    TMethod(FOnCreateDialog), P) then
...
procedure TCustomDCMemo.SetText(const V:
{$IFNDEF CLX}string{$ELSE}TCaption{$ENDIF});
begin
  Lines.Text := V;
end;
...
begin
  ...
  if (FClipboardType = cbRtf) and ((SyntaxParser = nil) or (SyntaxParser is
    TAttributeParser)) then
  begin
...
begin
  ...
  if ReadOnly then
    exit;
  if (FClipboardType = cbRtf) and RtfClipboard and ((SyntaxParser = nil) or
    (SyntaxParser is TAttributeParser)) then
  begin
...
begin
  ...
  NeedPopup := (Key >= #32) and Assigned(FOnHintPopup) and ((FPopupListBox = nil)
    or not (FPopupListBox.Visible));
  inherited;
{$IFDEF DCMBCS}
...
function TCustomDCMemo.ClosePopupListBox(var Key: Word; Shift: TShiftState):
  boolean;
begin
  result := false;
end;
...
procedure TCustomDCMemo.PopupBoxKeyDown(Sender: TObject; var Key: Word; Shift:
  TShiftState);
...
{--------------------------------------------}
 ...
 y1:Integer;
begin
      //self.Parent.parent.Parent.Parent.inhelp
//TEditForm(self.Parent.parent.Parent.Parent).inHelp
...
function TCustomDCMemo.DoFindInPopupBox(AStrings: TStrings; const s: string; var
...
begin
  ...
  s := _Copy(GetSource.Strings[CaretPoint.Y], FHintPos.X + 1, CaretPoint.X -
    FHintPos.X);

  pc:=self.Parent;
  while (pc<>Nil)and (not(pc.ClassNameIs('TEditForm'))) do
  pc:=pc.Parent;
//  if pc<>Nil then
//    TEditForm(pc).inHelp.FillPopupByFilterStr(TDCPopupListBox(Sender),s,fHintPopup);

  TPopupListBox(Sender).ItemIndex := _FindInStrings(FHintPopup, s);
end;

{--------------------------------------------}

procedure TCustomDCMemo.FindInPopupBox(Sender: TObject;BeginStr:String);
var
  Idx: integer;
  s: string;
begin
{  if Key = '.' then
  ...
  TPopupListBox(Sender).ItemIndex := _FindInStrings(FHintPopup,BeginStr);
end;
...
var
  ...
  AType: TPopupType;
begin
  result := Assigned(FOnHintPopup) and (FHintPos.X = CaretPoint.X) and
    (FHintPos.Y = CaretPoint.Y) and ActivatePopupBox(AType);
  if result then
  begin
...
procedure TCustomDCMemo.ShowPopupHint(const S: string; Point: TPoint; AutoHide:
  boolean);
begin
...
procedure TcustomDCMemo.ShowPopupListBoxEx2(Strings: Tstrings; Values:
  Tstrings);
var
  Pt: TPoint;
begin
  ...
  FPopupListBox := PopupListboxEx2(Self, FHintPopup, Values, Pt.X, Pt.Y +
    GetRealLineHeight(CaretPoint.Y),
    [lrLeftJustify, lrRightJustify], FTemplateFont, HintCallBack, 0);
  FPopupListBox.Sizeable:=true;  
...
procedure TCustomDCMemo.ShowPopupListBoxEx(Strings: TStrings; AlignArray: array
  of TLeftRightAlign; AMaxSize: integer);
var
  Pt: TPoint;
begin
  ...
  if FHintPopup <> Strings then
    FHintPopup.Assign(Strings);
  Pt := TextToPixelPoint(CaretPoint);
  FPopupListBox := PopupListbox(Self, FHintPopup, Pt.X, Pt.Y +
    GetRealLineHeight(CaretPoint.Y),
...
begin
  ...
  begin
    ...
    JumpTo(FHintPos.X+length(s)-1, FHintPos.Y);
...
begin
  ...
  if QEvent_type(Event) = QEventType_FocusIn then
    DoSetFocus
  else if QEvent_type(Event) = QEventType_FocusOut then
    DoKillFocus;
end;
...
function TCustomDCMemo.DoMouseWheel(Shift: TShiftState; WheelDelta: Integer;
...
begin
  ...
  begin
    ...
    if (FScrollBars = System.uitypes.TScrollStyle.ssHorizontal) or (fScrollBars = System.uitypes.TScrollStyle.ssBoth) then
...
begin
  GetSource.Navigate(GetMaxCaretLine - GetWBPosition(CaretPoint).Y + WinLine,
    0);
end;
...
begin
  ...
  begin
    ...
    if (moLineNumbers in FOptions) and ((GetRealGutterWidth = 0) or not
      (moLineNumbersOnGutter in FOptions)) then
      Invalidate;
  end;
end;
...
procedure TCustomDCMemo.InvalidateLines(FromLine, ToLine: integer; ParseOnly:
  boolean);
var
  R: TRect;
  MaxScrLine: integer;
...
begin
  ...
  if NeedSyntaxPaint then
    ToLine := Max(ToLine, GetSource.ParseToString(Min(ToLine, MaxScrLine), not
      FUseMonoFont));
...
begin
  ...
  begin
    ...
    try
      ...
      if SaveRect then
        SelectionRect := OldR
      else if [soOverwriteBlocks, soPersistentBlocks] * Options = [] then
        SelectionType := stNotSelected;
    finally
      DefaultAttribute := OldAttr;
      KeyState := 0;
...
begin
  ...
  result:=result and ((k in [32..126, 128..255])or (k>255));
end;
var
  OldAttr: integer;
  OldR: TRect;
...
begin
  ...
  //or (not (Key in [#32..#126, #128..#255])) then
  //green
  //условие ограничивало ввод кириллицы:
    exit;
...
begin
  ...
  begin
    ...
    else if BlockOption = bkBlockSel then
      BlockOption := bkNone;
    FWBlockChanged := true;
  end
  else if FWBlockChanged then
  begin
...
begin
  ...
  begin
    ...
    result := GetWBPositionEx(Point(Pos - 1, Line), false, false, Pos).Y -
      WinLine - 1;
    result := Max(0, result);
  end;
end;
...
{$ENDIF}
    ...
    else if (xa = 0) and (Abs(ya) = 1) then
    begin
{$IFNDEF CLX}
      if ya = 1 then
...
{$ENDIF}
  ...
  if (gw <> 0) and FGutterBackground.NeedPaint or (GutterBrush.Style <> bsSolid)
    then
  begin
...
procedure TCustomDCMemo.MakeSearchPointVisible(Rect: TRect; UseTopLeft:
  boolean);
begin
  if UseTopLeft then
  ...
  if (FrmSearRepl <> nil) and (FrmSearRepl.Visible) and (CurrentFindSource =
    GetSource) then
  begin
...
begin
  ...
  begin
    ...
    begin
      if FWordWrap and (GetWBPosition(Point(Pos - 1, Pt.Y)).X = 0) and (Pos - 1
        <> 0) and (Pos < _Length(s)) then
        inc(Pos);
{$IFDEF DCMBCS}
      if SysLocale.FarEast then
...
begin
  result := false;
  if HandleAllocated and ([csDesigning] * ComponentState = []) and Enabled and
    Focused then
  begin
    BLine := FCaretBaseLine;
    with GetCaretSize do
      if (FCaretSize.X <> X) or (FCaretSize.Y <> Y) or (BLine <> FCaretBaseLine)
        then
      begin
        FCaretSize.X := X;
        FCaretSize.Y := Y;
...
begin
  ...
  begin
    ...
    if (msParserChanged in State) and FCreated and (Self.ComponentState *
      [csReading, csLoading] = []) then
      SafeWordWrapLines(FirstChanged, LastChanged);

    if (msReadOnlyChanged in State) and (Color <> ReadOnlyColor) then
      Invalidate;

    if (ActiveMemo = Self) and ((msPositionChanged in State) or ((msEdited in
      State) and not FUseMonoFont)) then
    begin
      //UpdateCaretMode;
    ...
    else if (msInsState in State) and (ActiveMemo = Self) then
      UpdateCaretMode;
    ...
    begin
      NeedInvalidate := [msFoundRectChanged, msEdited, msBlockChanged,
        msNeedInvalidate, msParserChanged] * State <> [];
...
{$ENDIF}
         //green -  лаги при отрисовке номеров строк 
         Invalidate;
        //  QuickPaint(OWinChar, OWinLine);
          UpdateCaret;
        end;
      end
      else if NeedInvalidate then
        InvalidateLines(FirstChanged, LastChanged, false);
    end;

    if (msEdited in State) and (msCountChanged in State) and (FirstChanged >
      OldCount) and (moDrawSpecialSymbols in FOptions) then
      InvalidateLines(OldCount - 1, FirstChanged, false);

    if (msCountChanged in State) or (FHideScrollBars and
    ((FScrollBars=system.Uitypes.TScrollStyle.ssHorizontal) or(FScrollBars=system.Uitypes.TScrollStyle.ssBoth))
    and (State * [msEdited, msParserChanged] <> []))
        then
      UpdateScrollSize;
...
begin
  ...
  if (msPositionChanged in aState) then
    PaintCurLineNumber;
  //  PaintLineNumbers;

    if {(moBeginEndPaint in Options)and} not(MsParserChanged	in AState) then
  begin
    BeginEndPaint;
    Invalidate;
  end;
...
procedure TCustomDCMemo.Notification(AComponent: TComponent; Operation:
  TOperation);
begin
  inherited;
...
begin
  ...
  if (moLineNumbers in FOptions) and ((result = 0) or not (moLineNumbersOnGutter
    in Options)) then
    inc(result, FLineNumbersWidth + sDistLeftLineToText);
...
begin
  ...
  with FLineHighlight do
    if Visible then
      Update;
  StateChanged([msPositionChanged, msInsState, msCountChanged,
    msModifiedChanged, msObjNameChanged]);
end;
...
{$ENDIF}
  ...
  PaintLeftLine;
 // if moBeginEndPaint in Options then
  BeginEndPaint;
end;
...
function TCustomDCMemo.DoPaintGutter(const ARect: TRect; DrawState: TDrawState):
  boolean;
begin
  result := false;
...
{$IFNDEF CLX}
      BitBlt(Handle, R.Left, R.Top, R.Right - R.Left, R.Bottom - R.Top,
        FGutterBackGround.TempBackground.Canvas.Handle, R.Left, R.Top, SRCCOPY)
{$ELSE}
      QBitBlt(Canvas, R.Left, R.Top, FGutterBackGround.TempBackground.Canvas,
        R.Left, R.Top, R.Right - R.Left - 1, R.Bottom - R.Top)
{$ENDIF}
    else
    begin
...
begin
  ...
  try
    AssignPropsEx(GlobalMemoOptions, Self,
      ...
      'UseMonoFont,UseReadOnlyColor,ReadOnlyColor,SpecialSymbols');
    //don't resource
...
procedure TCustomDCMemo.DrawToFile(FileName: string; GraphicsFormat:
  TDCGraphicsFormat);
begin
...
function TCustomDCMemo.DoBeforePrint(const s: string; PrintType: TPrintType):
  string;
begin
  result := s;
...
{$ENDIF}
    ...
    PrintMemo.Options := PrintMemo.Options - [moDrawGutter, moDrawMargin,
      moLineNumbers];
  ...
  begin
    ...
    MetaFileRect := Rect(0, PrintMemo.LineHeight, Printer.PageWidth,
      Printer.PageHeight);
  ...
  begin
    if poShowProgress in PrintOptions then
      MemoPrintProgress.lblProgress.Caption := Format(SPrintingPage, [iPage + 1,
        IntToStr(cPages)]);
    Step;
    if iPage <> 0 then
      with PrintMemo do
    ...
    begin
      ...
      PageStr := DoBeforePrint(Format(SPageCaption, [iPage + 1, cPages]),
        ptFooter);
      GetTextExtentPoint32(Printer.Canvas.Handle, PChar(PageStr),
        Length(PageStr), TextSize);
      Printer.Canvas.Textout(Printer.PageWidth - (TextSize.cX) - TextSize.cX div
        Length(PageStr), 0, PageStr);
    end;
    Step;
  end;
  ...
  try
    ...
    begin
      ...
      begin
        Show;
        Application.ProcessMessages;
        lblPrinter.Caption := Format(SPrintingOnPrinter,
    ...
    if poShowProgress in PrintOptions then
      ...
      begin
        ...
        MemoPrintProgress.ClientHeight := btnCancel.Top + btnCancel.Height +
          ControlDist;
...
procedure TCustomDCMemo.CopyGraphicToClipboard(GraphicsFormat:
  TDCGraphicsFormat);
begin
  case GraphicsFormat of
...
procedure TCustomDCMemo.DrawToMetaFileEx(AMetaFile: TMetaFile; DC: THandle; AY:
  integer);
var
...
procedure TCustomDCMemo.PaintCurLineNumber;
var
  i, j: integer;
  y: integer;
  S,s1: string;
...
begin
  curPoint := CaretPoint;
  if CurPoint.Y = fPrevPoint.y then
    exit;
...
{$ENDIF}
    ...
    with TMSource(GetSource) do
    begin
      Pt := GetRealPosition(Point(0, CurPoint.Y));
      ...
      begin
        if (GetRealGutterWidth = 0) or not (moLineNumbersOnGutter in FOptions)
          then
          with ACanvas do
          begin
        ...
        CurPoint := GetCurentPosinPixels;
        CurPoint.X := 0;

{$IFNDEF CLX}

        if (fPrevPoint.Y <> -1) then
        begin
          if Trim(S)<>Trim(fPrevS) then
          begin
            if fPrevPoint.x = 5 then
              s1 := StringOfSpace(FLineNumbersLength - 1) + '-'
            else
              s1 := StringOfSpace(FLineNumbersLength - 1) + '.';
            ACanvas.TextOut(_GutterWidth, fPrevPoint.Y + Offset, S1);
          end;
        end;

        ACanvas.TextOut(_GutterWidth, CurPoint.Y + Offset, S);
{$ELSE}
        QTextOut(ACanvas, Rect(_GutterWidth, Y + Offset, _GutterWidth +
          FLineNumbersWidth, Y + Offset + AHeight), PChar(s),
...
{$ENDIF}
  ...
  if (Pt.Y = 0) or (((pt.Y + 1) mod 10) = 0) then
    fPrevPoint.Y := -1
  else
  begin
    fPrevPoint := CurPoint;
  ...
  if (((pt.Y + 1) mod 5) = 0) and (fPrevPoint.Y <> -1) then
  begin
    fPrevPoint.X := 5;
    fPrevPoint.Y := CurPoint.Y
  end;
  fPrevS:=s;
end;
    ...
    with TMSource(GetSource) do
      ...
      begin
        ///  if LineNumber<>0 then
         // Pt := GetRealPosition(Point(0, LineNumber)) else
        ...
        begin
          if (GetRealGutterWidth = 0) or not (moLineNumbersOnGutter in FOptions)
            ...
            begin
              Brush.Color := FLineNumBackColor;
              FillRect(Rect(_GutterWidth, Y, FLineNumbersWidth + _GutterWidth, Y
                + AHeight));
            end;
          Offset := Max((H - LineHeight) div 2, 0);

          //    aCanvas.MoveTo(FLineNumbersWidth + _GutterWidth,0);
            //  aCanvas.LineTo(FLineNumbersWidth + _GutterWidth, FDrawRect.Bottom);

          if (pt.Y = 0) or (((pt.Y + 1) mod 10) = 0) or (curPoint.Y = pt.Y) then
            //green
          begin
            if not ((moLimitLineNumbers in Self.FOptions) and (Pt.Y >=
              Lines.Count)) then
            begin
{$IFNDEF CLX}
...
{$ENDIF}
          ...
          begin
            if ((pt.Y + 1) mod 5) = 0 then
              s := StringOfSpace(FLineNumbersLength - 1) + '-'
            else
              s := StringOfSpace(FLineNumbersLength - 1) + '.';
            ACanvas.TextOut(_GutterWidth, Y + Offset, s);
          end;
...
procedure TCustomDCMemo.CalcGutterImgSize(Index: integer; var NewWidth,
  NewHeight: integer);
begin
...
{$IFNDEF CLX}
      TransparentBitBltEx(Canvas.Handle, GetDrawCanvas.Handle, Rect(0, 0, Width,
        Height),
        Rect(X, Y + CenterInc, X + NewWidth, Y + CenterInc + NewHeight),
        TransparentColor);
{$ELSE}
...
{$IFDEF DCMBCS}
              if SysLocale.FarEast then
                PaintBookMark(GetBookmarkFromRange(Index, _ByteToChar(s, OldPos)
                  - 1, _ByteToChar(s, Pos) - 1))
              else
...
{$ENDIF}
      ...
      begin
        PaintBookMark(TMSource(GetSource).GetBookmarkFromRange(Index, 0,
          MaxInt));
        Inc(Y, LineHeight);
      end;
    end;
...
{$ENDIF}
  with TMSource(GetSource) do
    ...
    begin
      ...
      else if GetRealLineVisible(i) then
      begin
...
procedure TCustomDCMemo.TempHighlightLineWithStyle(Index: integer; const
  StyleName: string);
var
  StyleIndex: integer;
begin
...
function TCustomDCMemo.FindWordBreak(LinePos: integer; var Pos, BaseLine:
  integer; var R: TRect): string;
var
  i: integer;
  idx: integer;
  Len: integer;
...
begin
  ...
  begin
    ...
    len := Max(SymbolsDrawn(LinePos, Pos, copy(S, Pos, length(s) - Pos + 1),
      copy(ColorS, Pos, length(ColorS) - Pos + 1),
...
{$ENDIF}
    ...
    if not FUseMonoFont then
      R.BottomRight := CalcStringSize(S, ColorS, Pos - 1, idx - Pos + 1,
        BaseLine);
...
function TCustomDCMemo.GetWordBreakCount(FromIndex, Pos, ToIndex: integer):
  integer;
var
  WB: TWBPoint;
  Index: integer;
...
procedure TCustomDCMemo.WordWrapLines(FromIndex, ToIndex: integer; AInvalidate:
  boolean);
var
  i: integer;
  Pos: integer;
...
begin
  ...
  begin
    ...
    begin
      s := Strings[i];
      if (s <> '') and ((FUseMonoFont and (length(s) > FWbCount)) or not
        FUseMonoFont) then
      begin
        Pos := 1;
        lpassed := false;
  ...
  WordWrapUpdated;
  AInvalidate := true;
  if ((H <> 0) and (H <> GetWrappedLineHeight(FromIndex))) or (LCount <>
    GetWordBreakCount(FromIndex, 0, ToIndex)) then
...
function TCustomDCMemo.GetRealPositionEx(Position: TPoint; CheckMBCS: boolean):
  TPoint;
var
  Index: integer;
  WB: TWBPoint;
...
function TCustomDCMemo.CalcWrapPos(LinePos, CharPos: integer; var APoint:
  TPoint): boolean;
var
  Len: integer;
  X: integer;
...
function TCustomDCMemo.GetRealLineIndex(LinePos: integer; var Pos: integer):
  integer;
var
  Index: integer;
  Wb: TWbPoint;
...
function TCustomDCMemo.GetWBPositionEx(Position: TPoint; CheckMBCS,
  ConvertResult: boolean; var Pos: integer): TPoint;
var
  Index: integer;
  Wb: TWBPoint;
  InsideLine: boolean;
...
{$ENDIF}
          ...
          InsideLine := (Index < FWordBreakList.Count - 1) and
            (PWBPoint(FWordBreakList[Index + 1])^.Y = Y);
        end;
    ...
    else if not InsideLine and HandleAllocated then
...
begin
  result := ((SyntaxParser <> nil) and (moColorSyntax in Options)) or
    HighlightUrls;
end;

{-------------------------------------------------------------}

procedure TCustomDCMemo.GetLineHeight(Sender: TObject; Index: integer; Item:
  TStringItem);
var
  Pt: TPoint;
  ABaseLine: integer;
...
procedure TCustomDCMemo.GetNavigation(Sender: TObject; var DeltaLine, DeltaChar:
  integer);
var
  APoint: TPoint;
  SavePoint: TPoint;
  ...
  begin
    if checkEof and (APoint.Y > GetRealStrCount + 1) then
      APoint.X := Min(APoint.X, GetWBStringLength(CaretPoint.Y, APoint.Y,
        true));
  end;
  ...
  begin
    with TMSource(GetSource), CaretPoint do
      ...
      begin
        ...
        if (DeltaLine <> 0) and (not (soBeyondFileEnd in Options)) and (Y +
          DeltaLine >= Lines.Count)
...
begin
  ...
  if AWrap or ((DeltaChar = 0) and (not FUseMonoFont or (soRetainColumn in
    GetSource.Options))) then
    _ChangePoint(AWrap, DeltaChar = 0)
  else
{$IFDEF DCMBCS}if SysLocale.FarEast and (DeltaChar = 0) then
...
{$ENDIF}
  ...
  begin
    with GetSource do
      if checkEof and (APoint.Y >= Strings.Count) and (GetRealStrCount >= 0)
        then
      begin
...
procedure TCustomDCMemo.CheckVisibleIndex(var Index: integer; AVisible:
  boolean);
begin
  if HideLines then
    if AVisible then
...
function TCustomDCMemo.GetWBStringLength(RealIndex, Index: integer;
  CheckLineEnd: boolean): integer;
var
  Wb: TWBPoint;
  ALine: integer;
  s: string;
...
begin
  ...
  with GetSource do
    ...
    begin
      ImageBit[CurLinePos, GutterImgIndex] := not ImageBit[CurLinePos,
        GutterImgIndex];
...
procedure TCustomDCMemo.DoGetFontFromStyle(Style: integer; Font: TFont; var
  Color: TColor);
begin
...
begin
  ...
  if not FUseTextStyle and (AParser <> nil) and AParser.GetBackColor(Style,
    result) then
  begin
    if result = -1 then
      result := Color;
...
procedure TCustomDCMemo.FontAndColorsFromStyle(Style: integer; var Font: TFont;
  var Color, BkColor: integer);
begin
...
begin
  result := (FUseTextStyle or (SyntaxParser = nil) or
    (SyntaxParser.GetFont(Style) = nil)) and
    ((Style >= 0) and (Style < TextStyles.Count) and
    TextStyles[Style].UpperCase);
end;

{-------------------------------------------------------------}

procedure TCustomDCMemo._FillRect(ARect: TRect; CurLine, CurWrapLine, CurChar,
  ABeginSel, AEndSel, AOldStyle: integer);
var
  R: TRect;
  R1: TRect;
...
{$IFNDEF CLX}
            BitBlt(Canvas.Handle, Left, Top, Right - Left, Bottom - Top,
              TempBackGround.Canvas.Handle, Left, Top, SRCCOPY);
{$ELSE}
            QBitBlt(Canvas, Left, Top, TempBackGround.Canvas, Left, Top, Right -
              Left, Bottom - Top);
{$ENDIF}
...
begin
  ...
  begin
    FUseTextStyle := true;
    try
      SetDCColor(GetFontColorFromTextStyle(AOldStyle),
        GetColorFromTextStyle(AOldStyle));
    finally
      FUseTextStyle := false;
    end;
  end;

  if not IsRectEmpty(R) and (not (moSelectOnlyText in FOptions) or
    (GetSource.SelectionType = stBlockSel)) then
  begin
    with ARect do
    begin
    ...
    begin
      if NeedPaintColor and not (moDisableInvertedSel in FOptions) then
        SetDCColor(GetColorFromTextStyle(AOldStyle),
          GetFontColorFromTextStyle(AOldStyle))
      else
...
begin
  ...
  result:=result or (curLine = CaretPoint.y);

  if (curLine = CaretPoint.y) then
  begin
  //highlight current line
    cl:=rgb(255, 255, 224);
    FLineHighlight.brush.color:=cl;
    FLineHighlight.pen.color:=clWhite;
  end else
  begin
...
procedure TCustomDCMemo.HighLightLine(ACanvas: TCanvas; CurLine: integer; R:
  TRect);
var
  OldWidth: integer;
  OldStyle: TPenStyle;
...
{$ENDIF}
      ...
      if FShape = shDoubleLine then
        DoLine(Point(Left, Top + Pen.Width - 1), Point(Right, Top + Pen.Width -
          1));
...
procedure TCustomDCMemo.PaintLeftLine;
var
  _GutterWidth: Integer;
  aCanvas: TCanvas;
  Rect: TRect;
begin
  ...
  begin
    ...
    if (moLeftIndent in FOptions) and (FLeftIndent > 0) then
      inc(_GutterWidth, FLeftIndent);
    //aCanvas.MoveTo(FLineNumbersWidth + _GutterWidth-sDistLeftLineToText,0);
    Rect.Left := FLineNumbersWidth + _GutterWidth - sDistLeftLineToText;
    Rect.Right := FLineNumbersWidth + _GutterWidth + sDistLeftLineToText;
    Rect.Top := 0;
    Rect.Bottom := FDrawRect.Bottom;
    aCanvas.Brush.Color := clWhite;
    aCanvas.FillRect(Rect);
    aCanvas.FloodFill(FLineNumbersWidth + _GutterWidth, FDrawRect.Bottom, clRed,
      fsSurface);
    aCanvas.MoveTo(FLineNumbersWidth + _GutterWidth, 0);
    aCanvas.LineTo(FLineNumbersWidth + _GutterWidth, FDrawRect.Bottom);
  end;
...
function TCustomDCMemo.CustomDraw(Canvas: TCanvas; const R: TRect; Ptr: PChar;
  Len: integer;
...
function TCustomDCMemo.GetCurFontHandle:
{$IFNDEF CLX}HFONT{$ELSE}QFontH{$ENDIF};
begin
...
procedure TCustomDCMemo.QFillRect(ACanvas: TCanvas; const R: TRect; ABrush:
  TBrush);
begin
...
procedure TCustomDCMemo.QTextOut(ACanvas: TCanvas; const R: TRect; Ptr: PChar;
  Len: integer; AFontInfo: PFontInfo; AFill: boolean; Buffer: Pointer);
var
  AFont: QFontH;
  AWidth: integer;
  i: integer;
...
procedure TCustomDCMemo.QBitBlt(dst: TCanvas; dx, dy: integer; src: TCanvas; sx,
  sy, sw, sh: integer);
begin
  dst.Start;
  src.Start;
  try
    ...
    bitBlt(QPainter_device(dst.Handle), dx, dy, QPainter_device(src.Handle), sx,
      sy, sw, sh, RasterOp_CopyROP, false);
  finally
    src.Stop;
    dst.Stop;
  end;
...
procedure TCustomDCMemo.DoTextOut(ACanvas: TCanvas; R: TRect; Style: integer;
  Ptr: PChar; Len: integer; Buffer: Pointer; OverHang: integer);
begin
  ...
  else if (OverHang > 0) and not UnderWindowsNT then
    with R do
...
procedure TCustomDCMemo._TextOut(var R: TRect; BaseLine, CurLine, CurWrapLine,
  CurChar,
  X, Y: integer; PStr: PChar; Len: integer; CalcRect, ASelection, AMarkedText:
  boolean);
var
  H: integer;
  OldRect: TRect;
...
{$IFDEF DCMBCS}
      if SysLocale.FarEast then
        Buffer := @FCharSp[0];
{$ENDIF}
      R.Right := R.Left + GetWidthAndInfo(GetCurFontHandle, Ptr, len, @result,
        Buffer);
      with result do
...
{$IFNDEF CLX}
      if (result.OverHang = 0) and ((FUseMonoFont and FFontMonoSpaced) or not
        FUseMonoFont) then
        Style := 0;
{$ENDIF}
      if Assigned(FOnCustomDraw) then
      begin
        ...
        if not CustomDraw(ACanvas, R, Ptr, Len, CurLine, CurChar, AStates +
          [dsBeforePaint]) then
        begin
{$IFNDEF CLX}
...
{$ENDIF}
          CustomDraw(ACanvas, R, Ptr, Len, CurLine, CurChar, AStates +
            [dsAfterPaint]);
        end;
      end
      else
...
{$ENDIF}
  ...
  begin
{$IFNDEF CLX}
    BitBlt(ACanvas.Handle, 0, 0, R.Right, R.Bottom,
      TempBackground.Canvas.Handle, X, Y, SRCCOPY);
{$ELSE}
...
begin
  ...
  begin
    ...
    try
      ...
      OutText(R, false, 0);
{$IFNDEF CLX}
      BitBlt(Canvas.Handle, X, Y, R.Right, R.Bottom, ACanvas.Handle, 0, 0,
        SRCCOPY);
{$ELSE}
...
{$ENDIF}
  ...
  begin
    ...
    if (ACanvas.Brush.Color = CheckReadOnlyColor(Color)) and
      NeedHighlightLine(CurWrapLine) then
    begin
      if IsHighlightBrushClear then
      begin
...
{$ENDIF}
    ...
    else if FUseMonoFont or (R.Right >= GetPaintX) then
      OutText(R, false, ETO_OPAQUE);
    dec(R.Right, overhang);
  end;
end;
...
{$ENDIF}
  if (a >= 0) and (b >= 0) then
    result := (ASkipColor or (GetColorFromTextStyle(a) =
      GetColorFromTextStyle(b))) and
...
function TCustomDCMemo.SetColorFromStyle(AColorStyle, AOldStyle, AOldColor,
  AOldBkColor: integer; InSelection, ASkipColor, ACalcRect: boolean): TFont;
var
  _color: integer;
  _bkcol: integer;
  _font: TFont;
begin
  if (AColorStyle = 0) and ((SyntaxParser = nil) or not (moColorSyntax in
    Options)) and (HighlightUrls) then
  begin
    _color := Font.Color;
    _bkCol := Color;
    _font := Font;
...
{$ENDIF}
  ...
  if (AOldBkColor <> 0) and (ASkipColor or (AOldBkColor <> {ColorToRgb(} Color))
    then ////
  begin
  ...
  else if InSelection or not ASkipColor then
    SetDCColor(_color, _bkcol);
  result := _font;
end;
...
procedure TCustomDCMemo.GetSelColorsFromStyle(Style: integer; var AColor,
  ABkColor: integer);
begin
...
function TCustomDCMemo.PaintString(var R: TRect; var BaseLine: integer; const S,
  ColorS: string;
  begChar, endChar, CurLine, CurWrapLine, CurPos, CurStyle: integer; SkipColors,
  CalcRect, CalcSymbols: boolean): integer;
var
  SLen: integer;
  curchar: integer;
  scolor: integer;
  ...
  begin
    ...
    begin
      ...
      if not (moColorSyntax in Options) and HighLightUrls and (result <> tokUrl)
        then
        result := 0;
      DoGetColorStyle(Point(cchar, CurLine), result);
      //result:= CaretPoint.y mod 256;
...
{$IFDEF DCMBCS}
  if (SysLocale.FarEast) and FUseMonoFont and (begChar > 0) and (_IsLeadByte(S,
    begChar)) then
  begin
    dec(CurChar);
  ...
  begin
    ...
    begin
      ...
      repeat
        ...
        while (ccount < Slen - curchar) and (_GetColor(curchar + ccount + 1) =
          scolor) do
        begin
          inc(ccount);
          if not CalcRect and InSelection(curchar + ccount + CurPos - 1, CurLine)
            <> _insel then
          begin
            selchanged := true;
            break;
          end;
          if not CalcRect and InFoundRect(curchar + ccount + CurPos - 1, CurLine)
            <> _MarkedText then
          begin
            MarkedChanged := true;
            break;
        ...
        if EqualStyles(scolor, ncolor, SkipColors or (_inSel and not selchanged)
          or _MarkedText) then
          scolor := ncolor
        else
          break;
      ...
      if SkipColors then
        FCurFont := SetColorFromStyle(scolor, CurStyle, OldColor, OldBkColor,
          _insel, SkipColors, CalcRect)
      else
        FCurFont := SetColorFromStyle(scolor, CurStyle, 0, 0, _insel,
          SkipColors, CalcRect);
...
{       if curLine = CaretPoint.y then
       begin
         clbak:=GetDrawCanvas.brush.color;
         cl:=rgb(245, 247, 210);
         GetDrawCanvas.brush.color:=cl;
         r1:=textRect;
         r1.right:=self.width;
         textRect:=r1;

         //GetDrawCanvas.brush.color:=clbak;

         //textrect.
          //SetDCColor(GetDrawCanvas.font.color, cl);
       end;
 }
        _TextOut(TextRect, BaseLine, CurLine, CurWrapLine, CurPos + curchar,
          Left, Top, @(s[curchar + 1]), ccount, CalcRect, _inSel, _MarkedText);
  ...
  else if CalcRect and not FUseMonoFont then
  ...
  //green TEst
//  if (fCurStyle = 6) or (fCurStyle = 7) then
//  begin
//    if AnsiContainsText(s, 'procedure') then
//    begin
//      self.Canvas.MoveTo(0, TextRect.Top);
//      self.Canvas.LineTo(100, TextRect.Top);
//    end;
//
//  end;
...
{-------------------------------------------------------------}
procedure TCustomDCMemo.BeginEndPaint;

procedure PaintLines(j: Integer; i: Integer;  tryy: TList; tryx: TList; BeginListX: TList; ACanvas: TCanvas);
var
  x: Integer;
  x1,x2:Integer;
  cl: TColor;
  ...
  fPosX:Integer;
begin
  PrevX := Integer(TryX.Items[TryX.Count - 1]);
  // beginListx.Count
  PrevY := Integer(TryY.Items[TryY.Count - 1]);
  // beginListx.Count
  TryX.Delete(TryX.Count - 1);
  TryY.Delete(TryY.Count - 1);
  p1.X := PrevX;
  p1.Y := PrevY;
  p1 := GetRealPosition(p1);
  p2.X := j;
  p2.Y := i;
  p2 := GetRealPosition(p2);
  PrevColor := aCanvas.Pen.Color;
  Counter := (TryX.Count) mod 10;
  cl := fBeginEndColors[Counter];
  acanvas.Pen.Color := cl;
  x1:= aCanvas.TextWidth(dupestring(' ', p1.X - 1))- WinChar * CharWidth;
  x2:= acanvas.TextWidth(dupestring(' ', (p2.X) - 1))- WinChar * CharWidth;
  if (x1<0)or (x2<0) then
  exit;
  x:=x1;
//  x := aCanvas.TextWidth(dupestring(' ', p1.X - 1))- WinChar * CharWidth;
  acanvas.MoveTo(GetPaintX + x, -WinLinePos * GetRealLineHeight(WinLinePos) + (p1.Y + 1) * GetRealLineHeight(p1.y) - 2);
  if p1.X > p2.X then
  begin
    x := x2; //acanvas.TextWidth(dupestring(' ', (p2.X) - 1))- WinChar * CharWidth;
    acanvas.LineTo(GetPaintX + x, -WinLinePos * GetRealLineHeight(WinLinePos) + (p1.Y + 1) * GetRealLineHeight(p1.y) - 2);
  end;
  acanvas.LineTo(GetPaintX + x, -WinLinePos * GetRealLineHeight(WinLinePos) + p2.Y * GetRealLineHeight(p2.y));
  if p2.X > p1.X then
  begin
    x := x2;//acanvas.TextWidth(dupestring(' ', (p2.X) - 1))- WinChar * CharWidth;
    acanvas.LineTo(GetPaintX + x, -WinLinePos * GetRealLineHeight(WinLinePos) + p2.Y * GetRealLineHeight(p2.y));
  end;
  acanvas.Pen.Color := PrevColor;
end;

var BeginListX,BeginListY,tryx,tryy,trycolor:TList;
...
begin
  ...
  if self.GetSource = nil then
  exit;

  BeginListX:=TList.Create;
  BeginListY:=TList.Create;
  tryx:=TList.Create;
  tryy:=TList.Create;
  tryColor:=TList.Create;
  counter:=0;
  try
    for I := 0 to self.Lines.Count-1 do
    begin
      s:=LowerCase(Self.Lines.Strings[i]);
      if AnsiContainsText(s,'try') then
      begin
        j:=AnsiPos('try',s);
        if j>0 then
        begin
          cs:=TmSource(self.GetSource).GetStoredColorData(i);
          if Length(cs)=0 then
          exit;
          k:=(byte(cs[j]));
          if k=6 then
      ...
      if AnsiContainsText(s,'begin')or
         AnsiContainsText(s,'finally')or
         AnsiContainsText(s,'except')or
         AnsiContainsText(s,'case')
      then
      begin
        j:=AnsiPos('begin',s);
        bool:=false;
        ...
        begin
          ...
          if k=6 then
          begin
            beginListX.Add(pointer(j));
            BeginListY.Add(Pointer(i));
            if (bool)and(tryx.Count>0) then
            PaintLines(j, i, tryy, tryx, BeginListX, ACanvas);
          end;
        end;
      end;
      ...
      begin
        ...
        if Length(cs)=0 then
          exit;
        k:=(byte(cs[j]));
        if (k=6)and(BeginListX.Count>0) then
          PaintLines(j,i,BeginListY,BeginListX,BeginListX,acanvas);
      end;
    end;
...
var
  ...
  var
    ...
    procedure PaintLine(const ALine, AClData: string; H, BLine, MaxPos:
      integer);
    var
      Pt: TPoint;
      R: TRect;
    begin
      ...
      begin
        ...
        begin
          ...
          PaintString(R, BLine, ALine, AClData, begChar, endChar, cline,
            CurLine, CurPos, lTextStyle, ASkipLineColors or not Enabled, false,
            false);
        end;
        ...
        begin
          ...
          _FillRect(Rect(R.Right, Y, A.Right + 1, Y + H), cLine, CurLine,
            CurPos, Pt.X, Pt.Y, lTextStyle);
        end;
      ...
      if moDrawLineBookmarks in Options then
      begin
        TMSource(GetSource).GetBookmarksFromRange(cline, CurPos - 1, MaxPos,
          FBookmarkList);
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                APos := Pos;
              PaintLine(Copy(AString, CurPos, Pos - CurPos), Copy(Cs, CurPos, Pos
                - CurPos), H, BLine, APos);
            end
          end
          else
            PaintLine(AString, Cs, GetRealLineHeight(i), GetBaseLine(i),
              MaxInt);
          ...
          begin
            ...
            begin
              ...
              PaintExpandCollapse(Canvas, PaintX - LHeight, OldY + AH, LHeight,
                AIndent, CollapseState[i] = csExpanded);
            end;
          end;
...
{$ENDIF}
  ...
  begin
    if HideLines then
    begin
      IntersectRect(ADrawRect, Rect(PaintX - LineHeight, 0, PaintX,
        ClientHeight), FDrawRect);
      if not IsRectEmpty(ADrawRect) then
        FDrawRect.Right := Max(FDrawRect.Right, PaintX + 1);
    end;
    IntersectRect(StringsRect, Rect(PaintX, 0, ClientWidth, ClientHeight),
      FDrawRect);
    OffSetRect(StringsRect, -PaintX, 0);
  end;

  OMarPos := (FMarginPos - WinChar) * CharWidth + PaintX;
  OPaintMar := (OMarPos >= Max(FDrawRect.Left, PaintX)) and (OMarPos <=
    FDrawRect.Right) and
...
begin
  ...
  result := (P.X < GetPaintX) or ((PosInBlock(Pt.X, Pt.Y) and not
    InValidPos(Pt.X, Pt.Y)));
  if result then
{$IFNDEF CLX}
...
var
  ...
  CRect: TRect;
begin
  if not HandleAllocated or ((FVertScrollBar = nil) and (FHorzScrollBar = nil))
    then
    exit;
...
var
  OldColor: TColor;
begin
  if (FHorzScrollBar <> nil) and (FVertScrollBar <> nil) and (Parent <> nil)
    then
    with BoundsRect do
    begin
      OldColor := Canvas.Brush.Color;
      Canvas.Brush.Color := TMWinControl(Parent).Color;
      Canvas.FillRect(Rect(Right - sScrollBarSize, Bottom - sScrollBarSize,
        Right, Bottom));
...
begin
  if Msg.TimerID = cDragDropTimer then
    DoSelTimer
  else if Msg.TimerID = cHintTimer then
  begin
    KillHintTimer;
    PopupHintBox;
  end;
...
procedure TDCGutterImages.AddFromResource(const ResName, GutterImageName:
  string;
  ABookMarkIndex: integer);
begin
...
begin
  ...
  fPrevPoint.Y := -1; //green
  setLength(fBeginEndColors,10);
  fBeginEndColors[0]:=clRed;
  fBeginEndColors[1]:=clMaroon;
  fBeginEndColors[2]:=clGreen;
  fBeginEndColors[3]:=clOlive;
  fBeginEndColors[4]:=clNavy;
  fBeginEndColors[5]:=clPurple;
  fBeginEndColors[6]:=clTeal;
  fBeginEndColors[7]:=clFuchsia;
  fBeginEndColors[8]:=clYellow;
  fBeginEndColors[9]:=clLime;
  //</green>
  ...
  FGutterImgs.AddFromResource('MEMO_BREAKPOINT', 'BreakPoint', -1);
  //don't resource
  FCompletion := TStringList.Create;
  FHintPopup := TStringList.Create;

  for i := 0 to 9 do
    FGutterImgs.AddFromResource('BIGBOOKMARK' + IntToStr(i), 'Bookmark' +
      IntToStr(i), i); //don't resource
...
var
  ...
  begin
    ...
    if result then
{$IFDEF CLX}
      result := (GetAveFontWidth(F) = ASize);
    // and (GetMaxFontWidth(F) = ASize);
{$ELSE}
      result := EqualFonts(Font, F, true) and (GetMinFontHeight(F) =
        GetMaxFontHeight(F));
{$ENDIF}
  end;
...
begin
  result := not (FWordWrap and FHideScrollBars and (GetWrapMargin > ClientWidth))
    and
...
{----------------------------------------}
  ...
  {----------------------------------------}
begin
  if Visible and CanFocus and TabStop and not (csDesigning in ComponentState)
    then
    SetFocus;
...
begin
  ...
  begin
    ...
    UpdateCaret;
    UpdateScrollPos;
    DoMemoScroll(system.Uitypes.TScrollStyle.ssHorizontal, I);
  end;
end;
...
begin
  if not HandleAllocated then
    exit;
{$IFNDEF CLX}
  if (FScrollBars = system.UITypes.TScrollStyle.ssHorizontal)or(FScrollBars = system.UITypes.TScrollStyle.ssBoth)// [ssHorizontal, ssBoth]
  then
  begin
    if CharScrollSize then
...
begin
  if not HandleAllocated then
    exit;
  {horizontal}
  if (FScrollBars=vcl.StdCtrls.ssHorizontal)or (fScrollBars =vcl.StdCtrls.ssBoth) then
  begin
    MaxPos := FHorzMaxPos;
...
{$ENDIF}
    if (FUseMonoFont and (AWidth <> GetMaxCaretChar)) or (not FUseMonoFont and
      (AWidth <> ClientWidth)) then
      WordWrapLines(0, MaxInt, true);
  end;
end;
  ...
  begin
    with TMSource(GetSource) do
      if recalc or not NeedMaxLineUpdate then
        UpdateMaxLine(FHideScrollBars and not FWordWrap and
        ((FScrollBars=vcl.StdCtrls.ssHorizontal)or
        (fScrollBars =vcl.StdCtrls.ssBoth)));
    UpdateScrollSize;
    UpdateScrollPos;
  end;
end;
...
begin
  ...
  if FWordWrap and not (moBreakWordAtMargin in Options) and
    ((FUseMonoFont and (GetMaxCaretChar <> OldWidth)) or (not FUseMonoFont and
    (ClientWidth <> OldWidth))) then
  begin
    with GetSource do
...
begin
  ...
  if not (soBeyondFileEnd in GetSource.Options) and (WinLine + i >= ACount + 1)
    and
...
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      if ((ARight > ALeft) or ((ARight = ALeft) and (ARight > 0))) and (ALeft <=
        X) and (ARight > X) then
      begin
        R := SelectionRect;
...
function TCustomDCMemo.IsUrlUnderPoint(Pt: TPoint; var s: string; TestOnly:
  boolean): boolean;
var
  Item: TStringItem;
  begPos: integer;
...
begin
  ...
  begin
    ...
    if result then
      with Item do
      begin
        result := (Pt.X >= 0) and (Pt.X < length(ColorData)) and
          (Byte(ColorData[Pt.X + 1]) = tokUrl);
        ...
        begin
          ...
          while (begPos > 1) and (Byte(ColorData[begPos - 1]) = tokUrl) do
            dec(begPos);
          while (endPos < length(ColorData)) and (Byte(ColorData[endPos + 1]) =
            tokUrl) do
            inc(endPos);
...
var
  s: string;
begin
  result := (GetSource.HighlightUrls) and IsUrlUnderPoint(ConvertMousePosEx(X,
    Y, false), s, true);
  {
...
begin
  inherited;
  if (FEditState = esNone) and (GetSource.HighlightUrls) and MouseAtHotString(X,
    Y) then
  begin
  ...
  else if Cursor = crMemoHandPt then
  begin
    Cursor := FOldCursor;
{$IFNDEF CLX}
...
{$IFDEF CLX}
  else if (FEditState = esNone) and not DoSetCursor then
    QWidget_setCursor(Handle, Screen.Cursors[Cursor]);
{$ENDIF}
end;
...
{--------------------------------------------}
  ...
  begin
    ...
    with GetSource do
      if moDblClickLine in Self.Options then
        MarkLine
      else
...
{$ENDIF}
  ...
  begin
    ...
    if not FMouseOnButton and ((Button = mbLeft) or not (moDisableRightClickMove
      in FOptions)) then
      SetPosByMouse(X, Y);
...
{$IFNDEF CLX}
      else if not (moDblClickLine in Self.FOptions) and (moTripleClick in
        Self.FOptions) and (GetTickCount - FDblClickTime < GetDoubleClickTime)
          and
        (X >= GetPaintX) and
        (Abs(FDblClicPoint.X - X) <= GetSystemMetrics(SM_CXDOUBLECLK)) and
        (Abs(FDblClicPoint.Y - Y) <= GetSystemMetrics(SM_CYDOUBLECLK)) then
      begin
        FEditState := esMarkWord;
        MarkLine;
        FDblClickTime := 0;
...
{$ENDIF}
      else if FEditState = esNone then
      begin
...
{$ENDIF}
    ...
    else if (Button = mbRight) and (FEditState in [esBlockDrag, esBlockSel])
      then
      KillTimer;
  end;
...
function TCustomDCMemo.CheckMouseOnButton(X, Y: integer; var Line: integer):
  boolean;
var
  AIndent: integer;
  LHeight: integer;
begin
  ...
  result := HideLines and (GetSource.CollapseState[Line] <> csNone) and
    (LineHeight <> 0);
  if result then
  begin
    LHeight := LineHeight;
...
{----------------------------------------}
  ...
  function AddItem(const ACaption: string; AEnabled: boolean; AOnClick:
    TNotifyEvent): TMenuItem;
  begin
    result := NewItem(ACaption, 0, false, AEnabled, AOnClick, 0,
      UniqueName(Self.Owner, 'MenuItem')); // don't resource
  end;
...
begin
  result := not (GetSource.Readonly) and (GetSource.CanPaste or ((FClipboardType
    = cbRtf)
{$IFNDEF CLX}
...
procedure TCustomDCMemo.SetClipboardBuffer(Format: Word; var Buffer; Size:
  Integer);
var
  Data: THandle;
  DataPtr: Pointer;
...
procedure TCustomDCMemo.SetClipboardBuffer(Format: string; var Buffer; Size:
  Integer);
var
  Stream: TMemoryStream;
...
begin
      ...
      case FEditState of
        ...
        esBlockDrag:
          begin
            ...
            else if not MoveBlockToPosEx(true) then
              SelectionType := stNotSelected;
          end;
        esBlockSel:
          begin
  ...
  if FMouseIsHot and (Button = mbLeft) and (FEditState = esNone) and
    (GetSource.HighlightUrls) then
  begin
...
function TCustomDCMemo.CollapseStateChanging(LinePos: integer; State:
  TCollapseState): boolean;
begin
...
procedure TCustomDCMemo.CollapseStateChanged(LinePos: integer; State:
  TCollapseState);
begin
...
procedure TCustomDCMemo.DoClickGutter(LinePos, ImageIndex: integer; Shift:
  TShiftState);
begin
  ...
  with GetSource do
    for i := 0 to cMaxGutterImages - 1 do
      ...
      begin
        ...
        if ((XCoord >= x) and (XCoord < X + NewWidth + 1) and (FGutterImgsAlign
          = iaLeft)) or
          ((XCoord <= x) and (XCoord > X - NewWidth - 1) and (FGutterImgsAlign =
          iaRight)) then
        begin
          result := i;
          exit;
...
procedure TCustomDCMemo.GetStrDataEx(var TextPoint: TPoint; var S, ColorS:
  string);
var
  Y: integer;
begin
...
function TCustomDCMemo.SymbolsDrawn(LinePos, CharPos: integer; const S, ColorS:
  string; var BaseLine: integer;
  var R: TRect; exact, CheckMBCS: boolean): integer;
var
  Pos: integer;
  ARight: integer;
...
begin
  ...
  ARight := R.Right;
  R.Bottom := 0;
  result := PaintString(R, BaseLine, S, ColorS, 0, length(S), 0, 0, 0, 0, false,
    true, true);
...
{$ENDIF}
  ...
  if FCurFont <> nil then
    ...
    begin
      ...
      dec(ARight, R.Right);
      i := ARight div Pos;
      if not exact and (abs(ARight - Pos * (i + 1)) < abs(ARight - Pos * i))
        then
        inc(i);
      inc(result, i);
    end
    ...
    begin
      ...
      i := GetCharsInWidth(GetCurFontHandle, Pchar(cs), Length(Cs), ARight,
        false, Exact, RealWidth);
{$IFDEF DCMBCS}
      if CheckMBCS and SysLocale.FarEast then
        i := _ByteToChar(cs, i);
{$ENDIF}
...
begin
  if FUseMonoFont then
    ...
    result := TextToPixelChar(Point(FromPos + Len, LineNo)) -
      TextToPixelChar(Point(FromPos, LineNo));
end;

{--------------------------------------------}

function TCustomDCMemo.CalcStringSize(S, ColorS: string; Pos, Len: integer; var
  BaseLine: integer): TPoint;
var
  R: TRect;
begin
  BaseLine := 0;
  ...
  R := Rect(0, 0, 0, 0);
  PaintString(R, BaseLine, S, ColorS, Pos, Pos + Len, 0, 0, 0, 0, false, true,
    false);
  with R do
...
function TCustomDCMemo.GetWrappedLineWidth(Index: integer; var Pos: integer;
  SingleLine: boolean): integer;
var
  WB: TWBPoint;
  AIndex: integer;
  Exact: boolean;
...
{$ENDIF}
    ...
    if (TextPoint.Y < Pt.Y) or ((TextPoint.Y = Pt.Y) and (TextPoint.X <= Pt.X))
      then
      result := -LineToPixel(TextPoint.Y, Pt.Y + 1, 1) +
        GetWrappedLineHeightEx(Pt.Y, Pos, CurPos, false, true)
    else
    ...
    begin
      ...
      begin
        ...
        if Pos <= Min(_CharToByte(s, TextPoint.X + 1), Length(S)) then
          /// last char <
          inc(result, tmp)
        else
        begin
          ...
          begin
            Pos := 1;
            inc(result, GetWrappedLineHeightEx(TextPoint.Y + 1, Pos, CurPos,
              true, true));
          end;
          break;
        end;
...
{$IFDEF DCMBCS}if SysLocale.FarEast then
        with TextPoint do
        begin
          s := Lines[Y];
...
function TCustomDCMemo.GetWrappedLineHeightEx(Index: integer; var Pos, BaseLine:
  integer; SingleLine, Direction: boolean): integer;
var
  WB: TWBPoint;
  AIndex: integer;
  found: boolean;
...
begin
  ...
  if not found then
    if Direction then
      Pos := Length(Lines[Index]) + 1
    else if (AIndex > 0) and (PWBPoint(FWordBreakList[AIndex - 1])^.Y = Index)
      then
...
function TCustomDCMemo.PixelToTextLine(var LinePos, CharPos: integer; CheckEnd:
  boolean): integer;
var
  H: integer;
  OldH: integer;
  ...
  begin
    result := ((LinePos >= 0) and (Pos <= len)) or ((LinePos < 0) and (Pos >
      0));
  end;
...
begin
  CharPos := 1;
  CheckEnd := CheckEnd and not FUseMonoFont and FWordWrap and not
    (soBeyondFileEnd in StringsOptions);
  if FUseMonoFont then
  begin
      ...
      repeat
        ...
        begin
          ...
          begin
            ...
            if LinePos < 0 then
              dec(CharPos);
            _inc(H, GetWrappedLineHeightEx(result, CharPos, BLine, true, LinePos
              >= 0));
            _inc(CurLine, 1);
            if LastLine then
            begin
...
function TCustomDCMemo.PixelToTextChar(LinePos, CharPos, X: integer; CheckEnd:
  boolean): integer;
var
  S: string;
  ColorS: string;
...
{$ENDIF}
  ...
  begin
    ...
    result := _ByteToChar(S, CharPos) - 1 + SymbolsDrawn(LinePos, CharPos,
      copy(S, CharPos, MaxInt), copy(ColorS, CharPos, MaxInt),
      BLine, R, exact, true);
...
function TCustomDCMemo.MouseToTextPoint(XX, YY: integer; CheckEnd: boolean):
  TPoint;
var
  LinePos: integer;
  Pos: integer;
begin
  ...
  begin
    ...
    if (LinePos < 0) then
      LinePos := 0
    else if (LinePos >= Lines.Count) and not (soBeyondFileEnd in
      GetSource.Options) then
...
function TCustomDCMemo.ConvertMousePosEx(XX, YY: integer; CheckEnd: boolean):
  TPoint;
begin
  if FWordWrap then
    ...
    with TMSource(GetSource) do
      ...
      begin
        ...
        if ((ARight > ALeft) or ((ARight = ALeft) and (ARight > 0))) and
          _CompareInt(Tag1, ARight, FSelDirection) and _CompareInt(ALeft - 1,
          Tag1, not FSelDirection) then
        begin
          with FSelStart do
            if not ((Y = Tag2) and (X >= ALeft) and (X <= ARight)) and
              InclusiveSel then
            begin
              if FSelDirection then
...
procedure TCustomDCMemo.ImportText(Sender: TObject; const s: string; AFont:
  TFont; AColor: TColor);
var
  len: integer;
begin
  ...
  begin
    ...
    if (DefaultAttribute <> 0) or (not EqualFonts(AFont, Font, false) or ((Color
      <> AColor) and (AColor <> -1))) then
      with FSelAttributes do
      begin
        BeginUpdate;
        try
...
procedure TCustomDCMemo.PasteFromImportStream(Stream: TStream; ImporterClass:
  TDCImporterClass);
var
  s: string;
  Sl: TStrings;
  ...
  try
    ...
    if not Importer.IsValidStream(Stream) then
      ...
      begin
        ...
        try
          Sl.LoadFromStream(Stream);
          InsertStrings(Sl, not ((BlockOption = bkStreamSel) or (BlockOption =
            bkNone)) and (ClipSelType = cfBlockSel));
        finally
          Sl.Free;
        end;
        SelectionType := stNotSelected;
...
procedure TCustomDCMemo.ExportToEx(const Exporter: TDCExporter; Selection:
  boolean);
var
  IsBold: boolean;
  IsItalic: boolean;
...
{$ENDIF}
  ...
  function _Get(Index: integer{$IFDEF DCMBCS}; Line: integer; AtEnd:
    boolean{$ENDIF}): integer;
  {$IFDEF DCMBCS}
  var
    s: string;
  {$ENDIF}
        ...
        case SelectionType of
          stStreamSel:
            if (Top = Bottom) or (Index = Bottom) then
              result := Min(result, _Get(Right{$IFDEF DCMBCS}, Index,
                true{$ENDIF}));
          stBlockSel:
            result := Min(result, _Get(Right + 1{$IFDEF DCMBCS}, Index,
              true{$ENDIF}));
        end;
  end;
        ...
        begin
          ...
          if AEnd >= AStart then
            repeat
              ...
              if (J <= AEnd) and EqualStyles(AStyle, Ord(ColorData[J]), false)
                then
              begin
                inc(J);
                continue;
              ...
              if (AStyle = 0) and ((SyntaxParser = nil) or not (moColorSyntax in
                FOptions)) and (HighlightUrls) then
              begin
                NewFont := Font;
                NewBkColor := Color;
              end
              ...
              IsFontName := (OldStrIndex = AStart) or (NewFont.Name <>
                OldFont.Name) or (NewFont.Name <> Font.Name);
              IsFontSize := (OldStrIndex = AStart) or (NewFont.Size <>
                OldFont.Size) or (NewFont.Size <> Font.Size);
              IsColor := (OldStrIndex = AStart) or (NewFont.Color <>
                OldFont.Color) or (NewFont.Color <> Font.Color);
...
procedure TCustomDCMemo.ExportToStream(Stream: TStream; Exporter:
  TDCExporterClass);
begin
...
procedure TCustomDCMemo.ExportToStreamEx(Stream: TStream; Exporter:
  TDCExporterClass; Selection: boolean);
var
...
procedure TCustomDCMemo.ExportToFile(const FileName: string; Exporter:
  TDCExporterClass);
begin
...
procedure TCustomDCMemo.ExportToFileEx(const FileName: string; Exporter:
  TDCExporterClass; Selection: boolean);
var
  Stream: TStream;
begin
...
procedure TCustomDCMemo.ExportToHtmlFileEx(const FileName: string; Selection:
  boolean);
begin
...
procedure TCustomDCMemo.ExportToRtfFileEx(const FileName: string; Selection:
  boolean);
begin
...
procedure TCustomDCMemo.ExportToHtmlStreamEx(Stream: TStream; Selection:
  boolean);
begin
...
procedure TCustomDCMemo.ExportToRtfStreamEx(Stream: TStream; Selection:
  boolean);
begin
...
begin
  ...
  begin
    ...
    if Index <> -1 then
      ...
      FPopupListBox := PopupListbox(Self, FCompletion, Point.X, Point.Y +
        LineHeight,
...
begin
  ...
  try
    ...
    AddStyleEx(STxtStComment, clGray, clWindow, [fsItalic], mcNone, false,
      true);
    ...
    AddStyleEx(STxtStMarkedBlock, clWhite, clNavy, [], mcSelection, false,
      false);
    AddStyleEx(STxtStSearchMatch, clLime, clBlack, [], mcSearchMatch, false,
      false);

    AddStyleEx(STxtStEmphasis, clBlack, clWindow, [fsBold], mcNone, False,
      True);
    ...
    AddStyleEx(STxtStScriptDelimiters, clBlack, clWindow, [], mcNone, False,
      True);
    AddStyleEx(STxtStScriptComment, clGray, clWindow, [fsItalic], mcNone, False,
      True);
    ...
    AddStyleEx(STxtStScriptResWord, clBlack, clWindow, [fsBold], mcNone, False,
      True);
...
function TTextStyles.AddStyleEx(const StyleName: string; AColor, ABackColor:
  TColor;
  AStyle: TFontStyles; AUpdateType: TUpdateMemoColors; AUseMemoFont,
  AUseMemoColor: boolean): TTextStyle;
begin
  BeginUpdate;
  try
...
function TTextStyles.AddStyle(const StyleName: string; AColor, ABackColor:
  TColor): TTextStyle;
begin
  BeginUpdate;
  try
...
begin
  ...
  else if (Collection = nil) or not TTextStyles(Collection).InColorUpdate then
    FColor := clWindow;
end;
...
begin
  result := integer(TStoredPosition(Item1).MemoSource) -
    integer(TStoredPosition(Item2).MemoSource);
end;
...
begin
  result := Integer(TMemoAttributes(Item1).Font.Handle) -
    Integer(TMemoAttributes(Item2).Font.Handle);
  if result = 0 then
    result := TMemoAttributes(Item1).Font.Color -
      TMemoAttributes(Item2).Font.Color;
  if result = 0 then
...
procedure TAttributeParser.AddToCachList(Attribute: TMemoAttributes; Index:
  integer);
begin
...
function TAttributeParser.GetBackColor(AState: integer; var Color: TColor):
  boolean;
begin
  result := true;
  with FFontManager do
...
begin
  with FFontManager do
    ...
    else if (Memo <> nil) and (AState <> tokUrl) then
      result := Memo.Font
    else
      result := nil;
end;
...
function TAttributeParser.ColorString(const StrData: ansistring; InitState: integer;
  var AColorData: ansistring): Integer;
var
  Item: TStringItem;
begin
...
procedure TAttributeParser.GetAttribute(APoint: TPoint; Attribute:
  TMemoAttributes);
begin
...
procedure TAttributeParser.PrepareColorData(LinePos: integer; const s: ansistring;
  var ColorData: ansistring);
begin
  if ColorData = '' then
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      else if FResetOnOverFlow or (Memo = nil) or (not Memo.AllowUndo) then
      begin
...
procedure TAttributeParser.GetStrAttribute(LinePos, Left, Right: integer; Index:
  byte;
  var ConsistentAttrs: TConsistentAttrs);
var
  i: integer;
...
procedure TAttributeParser.SetStrAttribute(LinePos, Left, Right: integer;
  Attribute: TMemoAttributes);
var
  s: string;
  fdata: string;
  ...
  procedure _SetStlye(var Styles: TFontStyles; Style: TFontStyle; AInclude:
    boolean);
  begin
    if AInclude then
...
{$ENDIF}
      ...
      else if (Right >= Left) and (Left <= length(s)) then
      begin
...
procedure TAttributeParser.GetAttributes(ARect: TRect; SelectionType:
  TSelectionType; Attribute: TMemoAttributes; var ConsistentAttrs:
  TConsistentAttrs);
var
  i: integer;
  Index: integer;
...
procedure TAttributeParser.SetAttribute(ARect: TRect; SelectionType:
  TSelectionType; Attribute: TMemoAttributes);
var
  //Index : integer;
  i: integer;
begin
...
begin
  ...
  if (FMemo <> nil) and (FMemo.SyntaxParser is TAttributeParser) then
    ...
    begin
      TAttributeParser(FMemo.SyntaxParser).SetAttribute(SelectionRect,
        SelectionType, Self);
      if (SelectionType <> stNotSelected) then
      begin
        ...
        try
          ...
          begin
            ...
            begin
              if StringItem[i] <> nil then
                StringItem[i].ItemState := StringItem[i].ItemState -
                  [isWasParsed];
              UpdateString(i);
            end;
          end;
...
begin
  ...
  else if Source is TMemoAttributes then
...
function TMemoAttributes.EqualFontStyles(A, B: TFontStyles; Style: TFontStyle):
  boolean;
begin
  result := ((Style in A) and (Style in B)) or (not (Style in A) and not (Style
    in B));
end;
...
function TMemoAttributes.GetAllAttributes(CheckStyle: boolean):
  TConsistentAttrs;
begin
  ...
  begin
    ...
    if (FMemo <> nil) and (FMemo.SyntaxParser is TAttributeParser) then
      with FMemo, GetSource do
        TAttributeParser(SyntaxParser).GetAttributes(SelectionRect,
          SelectionType, Self, FStoredAttrs);
...
initialization
  RegisterKeyboardInitProc(TCustomDCMemo, ResStr(SDefault),
    InitDefaultKeyboard);
  ...
  RegisterKeyboardInitProc(TCustomDCMemo, SKBDVisualStudio,
    InitVisualStudioKeyboard);
{$IFNDEF CLX}
  Screen.Cursors[crMemoHandPt] := LoadCursor(HInstance, 'MEMOHANDPT');
  //don't resource
