...
type
  ...
  public
    ...
    function getBreakPoints:TList;               //green
    procedure AddBreakPoint(LineNo:integer);
    procedure DelBreakPoint(LineNo:Integer);
...
implementation
uses
  replunit, searunit, searrepldlg, dcinpfrm, editUnit;
       ...
       begin
         ...
         for i := FromIndex to ToIndex do
           with FList[i] do
             ...
             begin
               s1:=fColorData;
               PrepareColorData(i, ansiString(StrData), s1);
               fColorData:=s1;
               FBeforeState := NewState;
               UpdateLinePos(i);
               s1:=fColorData;
               NewState := ColorString(StrData, NewState, s1);
               fColorData:=s1;
               FItemState := FItemState + [isWasParsed];
...
function TCustomMemoSource.getBreakPoints:TList;
begin
  result:=fBreakPointList;
end;

procedure TCustomMemoSource.AddBreakPoint(LineNo:integer);
var i:Integer;
begin
  i:=fBreakPointList.IndexOf(Pointer(LineNo));
  if i=-1 then
  fBreakPointList.Add(Pointer(LineNo));
end;

procedure TCustomMemoSource.DelBreakPoint(LineNo:Integer);
var i:Integer;
begin
  i:=fBreakPointList.IndexOf(Pointer(LineNo));
  if i>-1 then
...
begin
  ...
  try
    ...
    if (MRes = MrOk) or (Mres = mrNo+1) and (FLastSource <> nil)  then
      Replace;
  finally
    Free;
  end;
...
begin
  ...
  try
    ...
    if (ShowModal = mrOk) and (FLastSource <> nil) then
      Find;
    try
      if self.FActiveMemo<>Nil then
      if self.FActiveMemo.Visible then
      begin
        self.FActiveMemo.SetFocus;
        setParentsFocused;
      end;
    except
    end;
...
begin
  ...
  begin
    {$IFDEF DCMBCS}
    if SysLocale.FarEast and (s1[i] in LeadBytes) then
    begin
      inc(i, 2);
      continue;
    end;
    ...
    a := AnsiUpperCase(b);
    if a = b then
      S1[i] :=  ansiChar(AnsiLowerCase(b)[1])//green AnsiLowerCase(b)[1]
    else
      S1[i] := a[1];
    inc(i);
  end;
...
procedure TcustomMemosource.setParentsFocused;
//var pn:TWinControl;
begin
//  pn:=self.FActiveMemo;
//  while (pn<>nil) and (pn.Parent<>nil) do
//    pn:=pn.Parent;
//
//  if (pn<>nil) and (pn.ClassNameIs('TEditForm')) then
//  TEditForm(pn).setCodeFocused;
end;
...
begin
  ...
  try
    ...
    try
      ...
      begin
        if FirstSearch and (Origin = soFromCursor) and

        ((not IsPosInBlock(cChar, GetCurLine))and(not IsPosInBlock(cChar-1, GetCurLine)))
        //green
        //была бага с поиском в выделенном тексте
        //связано с некорректной работой функции ISPosInBlock
        //(X < ARight); -- должно быть <=
