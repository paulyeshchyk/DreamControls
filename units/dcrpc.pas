...
Type
  ...
  Public
    ...
    Property ClientCallEvent: TEvent read FClientCallEvent
      write SetClientCallEvent;
    Property ClientRetEvent: TEvent read FClientRetEvent
      write SetClientRetEvent;
    Property IsStubThread: Boolean read GetIsStubThread;
    Property ServerCallEvent: TEvent read FServerCallEvent
      write SetServerCallEvent;
    Property ServerRetEvent: TEvent read FServerRetEvent
      write SetServerRetEvent;
  End;
...
Begin
  FMutex.Get(Infinite);
  Try
    Result := Boolean(PAnsiChar(FSharedMem.FBuffer)[SizeOf(Integer)]);
  Finally
    FMutex.Release;
  End;
End;
...
Begin
  ...
  FMutex.Get(Infinite);
  Try
    Boolean(PAnsiChar(FSharedMem.FBuffer)[SizeOf(Integer)]) := True;
  Finally
    FMutex.Release;
  End;
End;
...
Begin
  ...
  Repeat
    ...
    Begin
      ...
      DataSize := RestSize;
    End;
    Move(PAnsiChar(FSharedMem.Buffer)[FPosition], PAnsiChar(@Buffer)[BufferPos],
      DataSize);
    Dec(Size, DataSize);
...
Begin
  FMutex.Get(Infinite);
  Try
    Boolean(PAnsiChar(FSharedMem.FBuffer)[SizeOf(Integer)]) := False;
  Finally
    FMutex.Release;
  End;
End;
...
Begin
  ...
  Repeat
    ...
    Else
      DataSize := RestSize;
    Move(PAnsiChar(@Buffer)[BufferPos], PAnsiChar(FSharedMem.Buffer)[FPosition],
      DataSize);
    Dec(Size, DataSize);
    Inc(FPosition, DataSize);
    Inc(BufferPos, DataSize);
...
Begin
  If FClientCallEvent.Wait(1) Then
    DoCall
  Else If FClientRetEvent.Wait(1) Then
  Begin
    LockRet;
    Try
      DoRet;
...
Begin
  Case FCallType Of
    ctNone:
      Error(SErrUnknownCallType);
    ctCall:
      FServerCallEvent.Signal;
    ctRet:
