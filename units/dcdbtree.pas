...
type
  ...
  public
    ...
    Id:integer;
    ParentId:integer;
    constructor Create(ObjId,Parent:Integer;Mark:TBookMark);
  end;
...
function  ValidBookmark(Dataset:TDataset;Mark:TBookMark):boolean;
...
function BookmarksCompare(Dataset:TDataset;Bookmark1,Bookmark2:TBookMark):integer;
begin
  {$IFDEF D3}
...
constructor TBookRec.Create(ObjId,Parent:Integer;Mark:TBookMark);
begin
  inherited Create;
  Id:=ObjId;
  ParentId:=Parent;
    ...
    with FDataset do
      begin
        ...
        try
          ...
          Filter:=OldFilter;
          if ValidBookmark(Fdataset,B) and (BookmarksCompare(Fdataset,BookMark, B) <>0) then
            BookMark:=B;
          EnableControls;
        end;
      end;
...
begin
  ...
  if ValidFields and (ObjId <> 0 ) then
    with FDataset do
      if ValidBookMark(Fdataset, TBookRec(ObjId).BookMark) then
      begin
        B:=BookMark;
        try
          ...
          if (BookmarksCompare(FDataset, BookMark , B) <>0) then
            BookMark:=B;
        end;
...
procedure TDCDataSetSource.Delete(ObjID:Integer);
  ...
  procedure DeleteWithChild(Id:integer;Mark:TBookMark);
  var
    B:TBookMark;
    curid : integer;
  begin
    ...
    begin
      ...
      FDataset.Delete;
      ObjectDeleted(curid);
      if ValidBookMark(Fdataset,Mark) then
        BookMark:=Mark;
    end;
  end;
...
begin
  ...
  if ValidFields and (Value <> 0 ) and (FUpdateCount <= 0)  then
    with FDataset do
      if  (ValidBookMark(FDataset,TBookRec(Value).BookMark))  and
          (BookmarksCompare(FDataset,BookMark,TBookRec(Value).BookMark) <> 0 ) then
        ...
        DsBrowse:
          begin
            ...
            try
              ...
              with FSelRec do
                if ( Pointer(BookMark) <> nil) and not ValidBookMark(Fdataset,BookMark) then
                begin
                  _DoAfterDelete(Id);
                  _DoSelectChanged(ObjectIndex);
                end
