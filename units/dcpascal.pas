...
*)
  ...
  public
    ...
    procedure   GetVarList2(S : TStrings);
...
begin
  DCRegisterType(WinTypeNames ,
                  [ TypeInfoPChar, TypeInfoPChar, TypeInfoPWideChar, TypeInfoArrayOfConst,
                    ...
                    //TypeInfo( TTokenInformationClass ),
                    TypeInfo( SECURITY_CONTEXT_TRACKING_MODE ),
...
begin
  ...
  if Dispatch.GetIDsOfNames(GUID_NULL, @AName, 1, 0, @fdispID) = S_OK then
    begin  //2204
      try
        if (TDelphiRunner(Dispatch)<>nil) then
        tdcScripter(TDelphiRunner(Dispatch).scripter.project).checkAndLoadformIfNeed(changeFileExt(TDelphiRunner(Dispatch).scripter.getDisplayScriptName,''));
      except
      end;
...
type
  ...
  public
    ...
    procedure loadFromString(str:Widestring);
  end;
...
//    procedure OpWithIntConst(ACode, Op1, constval : integer);
  ...
  public
    constructor Create(const ScriptStr : Ansistring; afout : TStream; Runner : TScriptRunner);
...
function TDeclConstList.toString:WideString;
...
begin
  ...
  try
    ...
    begin
      GetConstValue(Strings[i], val);
      flist.Values[Strings[i]] := vartoStr(val);
    end;
    result:=AnsiReplaceText(fList.Text, #13#10, cdecl_Separator);
  finally
    fList.Free;
  end;
end;

procedure TDeclConstList.loadFromString(str:Widestring);
var fList:TStringList;
i:integer;
sn,sv:String;
begin
  fList:=TStringList.Create;
  try
    fList.Text:=AnsiReplaceText(str, cdecl_Separator, #13#10);
...
procedure TDelphiRunner.GetVarList2(S : TStrings);
var
  i : integer;
  s1:String;
  id:TIdentListItem;
begin
  S.Clear;
    ...
    for i := 0 to THalCompiler(FHalCompiler).Variables.Count - 1 do
    begin

//      if (TIdentListItem(THalCompiler(FHalCompiler).Variables.Objects[i]).IdentType = tivariable) and (pos(coprocResult, Strings[i]) = 0) then
      begin
        S.Add(THalCompiler(FHalCompiler).Variables.Strings[i]);
{        THalCompiler(FHalCompiler).VarByName(THalCompiler(FHalCompiler).Variables.Strings[i],id);
        if id.Name<>EmptyStr
        then
        begin
         // s.Add('');
...
begin
  ...
  if Assigned(OnError) then
    ...
    begin
      ...
      FStop := (not Handled) or (scripter.terminateOnError);
   end;
end;
...
begin
  ValType:=R.NextValue;
  If (ValType=vaUTF8String)or(valType=vaString) then
    Result:=R.ReadString
  else
  if ValType=vaExtended then
  begin
    result:=r.ReadFloat;
  end else
  begin
...
begin
  ...
  with r do
    try
      ...
      //20122016 green
      str:=ReadWideString;
      fDeclConsts.loadFromString(str);
    finally
      r.Free;
    end;
end;
  ...
  with w do
    try
      ...
      with Prog do
        ...
        WriteWideString(fDeclConsts.toString);
    finally
      Free;
    end;
end;
...
var
  ...
  function convertToFloat():boolean;
  begin
    if (varIsStr(constValue))or (varISStr(val))then
    result:=false else
    begin
      result:=TryStrToFloat(varToStr(ConstValue), d1);
      result:=result and TryStrToFloat(varToStr(val), d2);
    end;
  end;

  function convertToInt():boolean;
  begin
    if (varIsStr(constValue))or (varISStr(val))then
    result:=false else
    begin
      result:=TryStrToInt(varToStr(ConstValue),i1);
      result:=result and TryStrToInt(varToStr(val), i2);
    end;
  end;

  begin
    //green
    //error typecasting variant-double
    //rm#9342
    case ACode of
      ocadd   :
        if convertToFloat then
            ConstValue := d1 + d2
          else
            ConstValue := ConstValue + Val;
      ocsub   :
        if convertToFloat then
          ConstValue := d1 - d2
        else
      ...
      ocdiv   :
        if convertToInt then
          ConstValue := i1 div i2
        else
          ConstValue := ConstValue div Val;
      ocmod   :
        if convertToInt then
        begin
          ConstValue := i1 mod i2;
        end
        else
          ConstValue := ConstValue mod Val;
      ocSlash :
        if convertToFloat then
          ConstValue := d1 / d2
        else
      ...
      ocshl   :
        if convertToInt then
        begin
          ConstValue := i1 shl i2;
        end
        else
          ConstValue := ConstValue shl Val;
      ocshr   :
        if convertToInt then
        begin
          ConstValue := i1 shr i2;
        end
        else
          ConstValue := ConstValue shr Val;
      ocor    :
        if convertToInt then
        begin
          ConstValue := i1 or i2;
          ...
          ConstValue := ConstValue or Val;
      ocxor   :
        if convertToInt then
        begin
          ConstValue := i1 xor i2;
        end else
        ConstValue := ConstValue xor Val;
      ocAnd   :
        if convertToInt then
        begin
          ConstValue := i1 and i2;
        end else
        ConstValue := ConstValue and Val;
      ocGreaterEqual   :
        if convertToFloat then
        ConstValue := d1 >= d2 else
        ConstValue := ConstValue >= val;
      ocEqual          :
        if convertToFloat then
        ConstValue := d1 = d2 else
...
begin
  ...
  TmpToken := self.ReadToken;
  if (TmpToken.stValue <> '.') and (TmpToken.stValue <> EmptyStr) then
    ErrSymbolExpected('.');
end;
...
Constructor THalCompiler.Create(const ScriptStr:AnsiString; afout: TStream;Runner:TScriptRunner);
Begin
  Inherited Create;
  FRunner := Runner;
  ...
  if afout <> nil then
    begin
      ...
      FParser.LinePtr:=PAnsiChar(ScriptStr);
    end;
End;
...
begin
  ...
  if lsize > 0 then
    begin
      ...
      list.Items[i]:=Pointer(sList^[i]);
//      Move(slist, @list, lsize * sizeof(pointer));
      FreeMem(slist);
    end;
end;
        ...
        if (ACode in [ocScriptInvoke, ocScriptInvoke2]) or (_dispatch.Count = 0) then
          begin
            if NamedItems.Find(AName, i) then
              begin
                tdcScripter(fRunner.Scripter.Project).checkAndLoadformIfNeed(aName);
...
{$IFDEF ACTIVESCRIPTING}
      ...
      begin
        ...
        if not FindExceptHandler then
          ...
          //green
        ThreadVars.fdispatch.Pop;
      end;
      {        //  if not(finfo.IsFinal) then exit; //green
}
