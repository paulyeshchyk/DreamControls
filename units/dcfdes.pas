...
{$ENDIF}
  ...
  public
    ...
    procedure GetMethods(TypeData: PTypeData; Proc: TGetStrProc);overload; {$IFNDEF D4}override;{$ENDIF}
    ...
    function  CreateMethod(const Name: string; TypeData: PTypeData): TMethod;Overload; {$IFNDEF D4}override;{$ENDIF}
    {$IFDEF USEVCLDESIGNER }
...
{$IFDEF D3}
    procedure SelectComponent(Instance: TPersistent);overload; {$IFNDEF D4}override;{$ENDIF}
    {$IFDEF USEVCLDESIGNER }
...
{$IFDEF USEVCLDESIGNER }
procedure TDCFormDesigner.SelectComponent(const ADesignObject: IDesignObject);
...
{$IFDEF USEVCLDESIGNER }
function TDCFormDesigner.CreateMethod(const Name: string; const AEventInfo: IEventInfo): TMethod;
var
  EvName : string;
  i      : integer;
begin
...
{$IFDEF USEVCLDESIGNER }
procedure TDCFormDesigner.GetMethods(const AEventInfo: IEventInfo; Proc: TGetStrProc);
var
  S : TStringList;
begin
...
begin
  if Assigned(OnStoreProp) then
   ...
   begin
     if (Instance is TCustomForm) and (PropInfo.Name='Visible') then
      store:=false;
   end;
end;
...
begin
  If not AnswerMessage then
    exit;
  sh:= forms.keysToShiftState(m.lparam); //TShiftState(M.lParam);
  If ssDouble in sh then
  begin
          ...
          begin
            ...
            try
              if Assigned(fWrapper.OnEvent) then
                fWrapper.OnEvent(PansiChar(EvName), cArgs + 1, npargs);
            finally
              FreeMem(npargs);
            end;
          end;
