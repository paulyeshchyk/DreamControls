...
uses
  windows,{OPP_Test_Form_Unit, }classes, forms, typinfo, sysutils, dcconsts, dcsystem,
  ...
  dcgen, dccommon, dcapp, dcfdes,dcdreamlib{, uOppForm,uOppProtokolForm};

type
  TIDEFormObject = class(TIDEObject)
  private
    //vadim FWithResHeader:boolean;
    FFormDesigner:IDCFormDesigner;
    FInstance:TComponent;
    FFormInstance:TCustomForm;
    procedure CreateDesigner(DataModule,AssignWrapper:boolean;AclassName:string='');
...
begin
//  Result:=True;//Vadim FFormDesigner.WasChanged;
...
procedure TIDEFormObject.CreateDesigner(DataModule,AssignWrapper:boolean;AclassName:string='');
var
...
begin
  ...
  begin
    ...
    begin
     ...
     // Finstance:=CreateFormWithClass(TTestForm);//CreateFormWithClass(TTestForm);//vadim{GetFormClassType});
     // FFormInstance:=TCustomForm(Finstance);
   //   FFormInstance:=CreateFormWithClass(TTestForm);
   //   Finstance:= TForm(FFormInstance);//TTestForm(FFormInstance).Panel1;
    //  Tform(FFormInstance).FormStyle:=fsMDIChild;
   if AssignWrapper then
    begin
//      Finstance:= CreateFormWithClass(TForm);
//      FFormInstance:=TForm(FInstance);

      _class:=GetClass(AclassName);
      if _class=nil then
        _class:=TCustomForm;

         Finstance:= CreateFormWithClass(TcomponentClass(_class));
       FFormInstance:=TcustomForm(FInstance);
    end
     else
      begin
...
//            FFormInstance:=TForm(FInstance);
  ...
  // FFormDesigner. LimitControl :=TTestForm(Finstance).panel1;
...
constructor TIDEFormObject.CreateNew(const FileName:TFileName; const FilerID:TFilerID;Dummy:Integer;Aclassname:string='');
{$IFDEF D3}
Var
...
begin
  ...
  isClosing:=false;
  FWithResHeader:=True;
  CreateDesigner(GetNewClassType.InheritsFrom(TDataModule),True,Aclassname);
...
//  RegisterSaveClass(TOPPSaveObject,TIDEFormObject,SExtDFM,SFormFile);
