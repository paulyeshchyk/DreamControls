...
type
  ...
  TTreeEditForm = class(TForm)
    ...
    procedure DCListViewEndDrag(Sender, Target: TObject; X, Y: Integer);
    procedure DCListViewDragOver(Sender, Source: TObject; X, Y: Integer;
      State: TDragState; var Accept: Boolean);
    procedure DCListViewDragDrop(Sender, Source: TObject; X, Y: Integer);
  ...
  private
    ...
    ListViewPrevSelectedIndex:Integer;
...
procedure TTreeEditForm.TreeViewDragOver(Sender, Source: TObject; X, Y: Integer;
  State: TDragState; var Accept: Boolean);
begin
end;
...
procedure TTreeEditForm.DCListViewDragDrop(Sender, Source: TObject; X,
  Y: Integer);
var si,di,newi:TListItem;
i:Integer;
prevY:Integer;
destIndex:Integer;
begin
  prevY:=-1;
  DestIndex:=-1;
  for I := 0 to TDCListView(Sender).Items.Count - 1 do
  begin
    if (y<=TDCListView(Sender).Items.Item[i].Position.Y+5)and
    (y>=PrevY) then
    begin
      DestIndex:=i;
      break;
    end;
    prevy:=TDCListView(Sender).Items.Item[i].Position.Y+5;
  end;
  if destIndex=-1 then
  destIndex:=TDCListView(Sender).Items.Count - 1;
  di:=TdcListView(Sender).Items.Item[destIndex];
  si:=TdcListView(Sender).Selected;
  if si=Nil then
  exit;
  if di=Nil then
  exit;
  if si.Index=di.Index then
  exit;
  if si.Index>di.Index then
  begin
   // newi:=TListItem.Create(TListItems(TDCListView(Sender).Items));
   // newi.Assign(si);
//    for I := di.Index to si.Index-1 do
//    begin
//      TdcListView(Sender).Items.Item[i+1]:=TdcListView(Sender).Items.Item[i];
//    end;
    si.Caption:=di.Caption;
    si.Data:=di.Data;
    si.ImageIndex:=di.ImageIndex;
    si.Indent:=di.Indent;
    si.OverlayIndex:=di.OverlayIndex;
    si.StateIndex:=di.StateIndex;
    si.SubItems:=di.SubItems;
    si.Checked:=di.Checked;
    ...
    //TDCListView(Sender).Items.Item[di.Index].Assign(si);
  end else
  begin
...
procedure TTreeEditForm.DCListViewDragOver(Sender, Source: TObject; X,
  Y: Integer; State: TDragState; var Accept: Boolean);
begin

  ListViewPrevSelectedIndex:=-1;
  Accept:=false;
  if Sender.ClassNameIs('TDCListView') then
  if Source.ClassNameIs('TDCListView') then
  begin
    Accept:=true;
    if TDCListView(Sender).Selected<>Nil then
    ListViewPrevSelectedIndex:=TDCListView(Sender).Selected.Index;
  end;
end;

procedure TTreeEditForm.DCListViewEndDrag(Sender, Target: TObject; X,
  Y: Integer);
begin
  try
    RefrButClick(Sender);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Columns[0].Width := -1;
        Columns[0].Caption:='Наименование объекта';
        ...
        ViewStyle := vsReport;
       //Vadim ShowColumnHeaders := ACount <> 0;
      end;
    end;
