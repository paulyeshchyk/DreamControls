...
type
  ...
  TEventHandler = function (EventName : pAnsiChar; cArgs : integer; pArgs : PVariantArgList) : variant of object;
...
{$ENDIF} // WIN
  ...
  public
    ...
    //vadim
    function GetFileInfo(const FileName: TFileName; Mode: Integer): String;virtual;
...
{$ENDIF}
  ...
  TDBFileSystem=class(TFileSystem)
  public     //vadim
    function GetCurrentDir:String;override;
    procedure SetCurrentDir(const Dir:String);override;
    function GetFileStream(const FileName: TFileName; Mode: Integer): TStream;override;

    function GetCompiledFileStream(const FileName:TFileName;Stream:TStream):Boolean; //green
    procedure WriteCompiledFileStream(const FileName:TFileName; Stream:TMemoryStream);// green
    ...
    function FileExists(const FileName: TFileName): Boolean;override;
    function GetTempFileName(const FileName: TFileName): TFileName; override;
    function GetFileInfo(const FileName: TFileName; Mode: Integer): String;override;
  end;
...
{$ENDIF}
  ...
  public
    ...
    function ColorString(const StrData: AnsiString; InitState: Integer;
      var AColorData: AnsiString): Integer; virtual; abstract;

    function  StoreColorData : boolean; virtual;
    procedure PrepareColorData(LinePos : integer; const s : ansistring ; var ColorData : Ansistring); virtual;
...
//vadim
function DBFileSystem:TDBFIleSystem;
// -------------------- OLE support --------------------
{$IFDEF WIN}
{$IFNDEF CLX}
...
function  PReadShortStr(var p : pAnsiChar) : AnsiString;
...
procedure StrArrayToStrings(Strings:TStrings;const A:Array of Ansistring);
...
{$ENDIF}
  ...
  FilerSignature: array[1..4] of AnsiChar = 'TPF0'; // don't resource
  ...
  StringTypes:TTypeKinds=[tkString,tkWChar, tkLString{$IFDEF D3} ,tkWString {$ENDIF}, tkUString];
...
function RemoveNonIdentChars(const N : string) : string;
begin
  result := RemoveCharSet([#0..#255] - ['a'..'z','A'..'Z','_','À'..'ß','à'..'ÿ'], N);
end;
...
function StringToCharSet(InitValue:TCharSet;const S:String;CaseSens:Boolean):TCharSet;
  ...
  begin
    For i:=1 to Length(TempStr) do
      Include(Result,ansiChar(TempStr[i]));
  end;
      ...
      try
        With TVarData(VarValue) do
          Case VType of
            ...
            begin
              if reg.ValueExists(fName) then
              begin
                if (reg.GetDataType(fName) = rdString)or (reg.GetDataType(fName)=rdExpandString) then
...
//  With InternalRegistry do
    if reg.OpenKey(FKey,False) then
      try
        if reg.ValueExists(FName) then
        begin
          regDataType := reg.GetDataType(fName);
          //rdUnknown, rdString, rdExpandString, rdInteger, rdBinary
          With TVarData(DefaultValue) do
            Case VType of
              varSmallint,varInteger,VarByte:
                if (regDataType = rdInteger)or (regDataType = rdBinary) then
                Result:=reg.ReadInteger(FName);
              varDouble,varSingle:
                Result:=reg.ReadFloat(FName);
              varCurrency:
                Result:=reg.ReadCurrency(FName);
              varDate:
                Result:=reg.ReadDate(FName);
              varBoolean:
                Result:=reg.ReadBool(FName);
              varString,varOleStr, varUString:
              if (regDataType = rdString)or (regDataType = rdExpandString) then
                Result:=reg.ReadString(FName);
...
begin
  ...
  for i := 1 to DisplayPropNameCount do
    begin
      ...
      if (pinfo = nil) or not (GetPTypeInfo(pinfo).kind in [tkString,tkWChar, tkLString, tkUString]) then
        continue;
...
begin
  ...
  try
    ...
    try
      Result := {ExpandFileName}(RelativeName);//vadim  AddSlash(FSystem.GetCurrentDir) + ExtractFileName(RelativeName);
    finally
...
Begin
  ...
  Case HorzAlignment Of
    ...
    haCenter:
      With Result Do
      Begin
        result.Left:=(ARectOut.Right+ARectout.Left-Size.cx) Div 2;
        Right:=Left+Size.cx;
      End;
    haRight:
      With Result Do
      Begin
        Left:=ARectOut.Right-Size.cx;
  ...
  Case VertAlignment Of
    vaLeft:
      With Result Do
      Begin
        Result.Bottom:=ARectOut.Top+Size.cy;
    ...
    vaCenter:
      With Result Do
      Begin
        Top:=(ARectOut.Bottom+ARectout.Top-Size.cy) Div 2;
        Bottom:=Top+Size.cy;
      End;
    vaRight:
      With Result Do
      Begin
        Top:=ARectOut.Bottom-Size.cy;
...
//Vadim
function TFileSystem.GetFileInfo(const FileName: TFileName; Mode: Integer): String;
begin
  result := FileName;
end;
...
//======================================================
//vadim
function TDBFileSystem.GetCurrentDir:String;
begin
  result:='';
end;
procedure TDBFileSystem.SetCurrentDir(const Dir:String);
begin

end;

function TDBFileSystem.GetFileInfo(const FileName: TFileName; Mode: Integer): String;
//var
//  fn:string;
begin
//  result:='';
//  fn:=ExtractFileName(filename);
//  result:=OPPGetFileSystemInfo(fn,mode);
  result := FileName;
end;

function TDBFileSystem.GetFileStream(const FileName: TFileName; Mode: Integer): TStream;
// var
//  fTaskID:integer;
//fn:string;
begin
//  fTaskID:=-1;
//  if Assigned(OPPGetTaskID) then
//    fTaskID:=OPPGetTaskID;
//  result:=OPPGetFileStream(fTaskID,FileName,Mode);
  result := TMemoryStream.Create;
end;

function TDBFileSystem.GetCompiledFileStream(const FileName: TFileName; Stream: TStream):Boolean;
begin
//  result:=OPPGetCompiledStream(FileName,Stream);
  result := false;
end;

procedure TDBFileSystem.WriteCompiledFileStream(const FileName: TFileName; Stream: TMemoryStream);
begin
//  OPPSaveCompiledStream(FileName, Stream);
end;



function TDBFileSystem.FileAge(const FileName: TFileName): Integer;
begin
// result:=OPPFileAge(ExtractFileName(FileName));
  result := 0;
end;

function TDBFileSystem.RenameFile(const OldName, NewName: TFileName): Boolean;
begin
// result:=OPPRenameFile(ExtractFileName(OldName), ExtractFileName(NewName));
  result := false;
end;

function TDBFileSystem.IsReadonly(const FileName: TFileName): Boolean;
//var fn:String;
begin
// fn:=ExtractFileName(filename);
// result:=OPPisReadOnly(fn);
  result := false;
end;
function TDBFileSystem.DeleteFile(const FileName: TFileName): Boolean;
// var
//
//fn:string;
//begin
...
function TDBFileSystem.FileExists(const FileName: TFileName): Boolean;
//var
//fn:string;
begin
// result:=False;
// fn:=ExtractFileName(filename);
// result:=OPPFileExists(fn);
  result := false;
end;

function TDBFileSystem.GetTempFileName(const FileName: TFileName): TFileName;
begin
//  result:=OPPGetTempFileName(ExtractFileName(fileName));
  result := ExtractFileName(fileName) + '.tmp';
end;
...
function DBFileSystem:TDBFIleSystem;
begin
  If FDBFileSystem=nil then
    FDBFileSystem:=TDBFileSystem.Create(nil);

  Result:=FDBFileSystem;
end;
...
begin
  Result:='';//vadim SysUtils.GetCurrentDir;
end;
...
begin
  If AFileSystem=Win32FileSystem then
    exit;
      If AFileSystem=DBFileSystem then
    exit;
  With FFileSystemList do
    for i:=Count-1 downto 0 do
...
procedure TSimpleParser.PrepareColorData(LinePos : integer; const s : ansistring ; var ColorData : ansistring);
var
  Leng : integer;
begin
  Leng := Length(s);
...
begin
  ...
  SafeRegisterClass(TDBFileSystem);
{$IFDEF WIN}
{$IFNDEF CLX}
...
begin
  for i:=1 to 12 do
    if CompareText( FormatSettings.ShortMonthNames[i],ShortMonthName)=0 then
    begin
      Result:=i;
      exit;
    end;
...
begin
  ...
  try
    ...
    Result :=ARelativePath; //vadim ExpandFileName(ARelativePath);
  finally
...
{$ENDIF}
  ...
  FreeObject(FDBFileSystem,FDBFileSystem);
